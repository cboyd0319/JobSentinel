...
class TestWindowsDeploymentCore:
    ...
    def test_python_version_check(self):
        ...
        assert version >= (3, 11), f"Python {version.major}.{version.minor} < 3.11 (minimum required)"
        
        # Recommended version for Windows
        assert version >= (3, 12), "Python 3.12+ recommended for Windows 11"
    ...
    def test_config_validate_command(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "jsa.cli", "config-validate",
             "--path", "config/user_prefs.example.json"],
            capture_output=True,
            text=True,
            timeout=10
        )
...
class TestWindowsEndToEnd:
    ...
    @pytest.mark.skipif(
        not Path("config/user_prefs.json").exists(),
        reason="Requires config/user_prefs.json"
    )
