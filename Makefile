...
help:
	...
	@echo '  make test-core   - Run tests for new core (deploy/common/app/src/jsa)'
	@echo '  make test-fast   - Alias of test'
	@echo '  make test-all    - Run full + optional example tests (RUN_EXAMPLE_TESTS=1)'
	@echo '  make lint        - Ruff lint (deploy/common/app/src/jsa, deploy/common/tests/unit_jsa)'
	@echo '  make type        - mypy type check (deploy/common/app/src/jsa)'
	@echo '  make fmt         - Run black formatter'
	@echo '  make cov         - Coverage for deploy/common/app/src/jsa (fail-under=85)'
	@echo '  make mut         - Run mutation tests (if mutmut installed)'
	@echo '  make security    - Run PyGuard security & quality checks'
	@echo '  make analyze     - Run sample ATS analysis (deploy/common/examples/sample_resume.txt)' 
...
test:
	$(PYTHON) -m pytest -q deploy/common/tests
...
test-all:
	RUN_EXAMPLE_TESTS=1 $(PYTHON) -m pytest -q deploy/common/tests
...
mut:
	@if command -v mutmut >/dev/null 2>&1; then \
	  mutmut run --paths-to-mutate deploy/common/app/src/jsa --tests-dir deploy/common/tests; \
	else \
...
security:
	...
	@if command -v pyguard >/dev/null 2>&1; then \
	  for file in deploy/common/app/src/jsa/*.py; do \
	    echo "Scanning $$file..."; \
...
analyze:
	python deploy/common/scripts/ats_cli.py scan --resume deploy/common/examples/sample_resume.txt || echo 'Provide sample resume at deploy/common/examples/sample_resume.txt'
