...
def check_python_version():
    ...
    if python_version < (3, 8):
        print(f"❌ ERROR: Python {python_version.major}.{python_version.minor} is too old")
        ...
        print(f"✅ Python {python_version.major}.{python_version.minor}.{python_version.micro} is OK")
        return True
...
def check_and_create_config():
    ...
    if not example_config.exists():
        print("❌ ERROR: Missing example configuration file")
        print("   This might be a corrupted installation")
        return False
    
    if not user_config.exists():
        print("⚠️  You need to create your personal configuration")
        ...
        if answer in ['y', 'yes']:
            ...
            print("❌ You need a configuration file to continue")
...
def check_and_create_env():
    ...
    if not env_example.exists():
        print("❌ ERROR: Missing .env.example file")
        return False
    
    if not env_file.exists():
        print("⚠️  You need to create your environment file")
        ...
        if answer in ['y', 'yes']:
            ...
            print("❌ You need an .env file to continue")
    ...
    if answer in ['y', 'yes']:
        ...
        try:
            ...
            if result.returncode == 0:
                print("✅ Slack setup completed!")
            else:
                print("⚠️  Slack setup had issues, but you can try again later")
        except Exception as e:
            print(f"⚠️  Could not run Slack wizard: {e}")
...
def offer_test_run():
    ...
    if answer in ['y', 'yes']:
        ...
        try:
            ...
            print("⚠️  Test timed out (this can happen on first run)")
...
⚠️  REMEMBER: This is ALPHA software
