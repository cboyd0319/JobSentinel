...
} from "./macos-readiness-workflow.mjs";
...
export function evaluateMacosReadiness({ root = defaultRoot, env = process.env } = {}) {
  ...
  const buildTests = read(root, "scripts/tests/platform/build-macos-dmg.test.mjs");
  const latestVerifierTests = read(
    root,
    "scripts/tests/release/verify-latest-macos-release.test.mjs",
  );
  const packageVerifierTests = read(
    root,
    "scripts/tests/release/verify-macos-package.test.mjs",
  );
  ...
  const criteria = [
    criterion(
      ...
      scripts["tauri:build:macos"] === "node scripts/platform/build-macos-dmg.mjs" &&
        ...
        scripts["macos:readiness"] === "node scripts/release/check-macos-readiness.mjs",
