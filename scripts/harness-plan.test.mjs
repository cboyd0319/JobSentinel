...
test("plans script tests and focused script test", () => {
  withFixture((root) => {
    writeFixtureFile(root, "scripts/harness-plan.test.mjs");
    ...
    assert.deepEqual(commandsFor(plan), [
      ...
      "node --test scripts/harness-plan.test.mjs",
    ]);
  });
});
...
test("plans E2E smoke and strict browser doctor for Playwright harness changes", () => {
  withFixture((root) => {
    writeFixtureFile(root, "scripts/run-playwright.test.mjs");
    ...
    assert.deepEqual(commandsFor(plan), [
      "npm run harness:check",
      "npm run test:scripts",
      "node --test scripts/run-playwright.test.mjs",
      "npm run doctor:e2e",
