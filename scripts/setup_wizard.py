...
class Colors:
    ...
    def supports_color() -> bool:
        ...
        return (
            ...
            platform.system() != 'Windows' or os.environ.get('TERM', '').lower() != 'dumb'
        )
...
def main() -> int:
    ...
    locations_input = ask_input("Locations (comma-separated, e.g., 'Remote, San Francisco')", "Remote")
    if locations_input:
    ...
    if ask_yes_no("Enable Reed.co.uk (requires free API key)?", default=False):
        ...
        config["job_sources"]["reed"] = {
            "enabled": bool(reed_key),
            "api_key": reed_key
        }
    ...
    if ask_yes_no("Enable resume optimization features?", default=True):
        industries = [
            "software_engineering", "data_science", "devops", "cybersecurity",
            "healthcare", "finance", "legal", "education",
            "marketing", "sales", "product_management", "design"
        ]
