...
def extract_findings_from_json(data: Any) -> List[Dict[str, Any]]:
    ...
    if findings and len(findings) > 0:
        print(
            f"DEBUG: First finding keys: {list(findings[0].keys()) if isinstance(findings[0], dict) else 'Not a dict'}"
        )
...
def build_sarif_from_json(json_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for finding in findings:
        ...
        is_security_finding = (
            status == "new"
            and status_id == "1"  # OCSF new finding
            or status_code == "FAIL"  # Explicit fail
            or status in {"fail", "failed", "failure"}
            or activity_name == "create"  # Created/detected finding
        )

        if not is_security_finding:
            if len(results) < 3:  # Debug first few non-security findings too
                print(
                    f"DEBUG: Skipping finding - not a security finding (status: '{status}', status_id: '{status_id}')"
                )
            continue
        ...
        rule_id = finding.get("check_id") or finding_info.get("uid") or finding.get("uid") or "prowler-finding"

        rule_name = finding_info.get("title") or finding.get("check_title") or finding.get("title") or rule_id

        rule_desc = (
            finding_info.get("desc")
            or finding.get("check_description")
            or finding.get("description")
            or status_detail
            or "Prowler security finding"
        )
        ...
        if rule_id not in rules:
            rules[rule_id] = {
                ...
                "properties": {"tags": ["security", "prowler"]},
            }
        ...
        message_text = (
            ...
            or f"{rule_name}: Security finding detected"
        )

        # Build properties
        properties = {"status": status, "severity": severity}
        ...
        result = {
            ...
            "locations": [
                {"physicalLocation": {"artifactLocation": {"uri": "repository"}}, "message": {"text": location_text}}
            ],
        }
...
def main() -> None:
    ...
    if len(sys.argv) != 3:
        raise SystemExit("Usage: convert_prowler_json_to_sarif.py <input.json> <output.sarif>")
