...
function collectFeatureMockDirectories(root, dir = join(root, "src/features")) {
  const directories = [];
  ...
  for (const entry of readdirSync(dir, { withFileTypes: true })) {
    ...
    const fullPath = join(dir, entry.name);
    if (entry.name === "mocks") {
      directories.push(relative(root, fullPath).replaceAll("\\", "/"));
      continue;
    }

    directories.push(...collectFeatureMockDirectories(root, fullPath));
  }
...
export function checkFrontendBoundaries(root = defaultRoot) {
  ...
  const violations = collectFeatureMockDirectories(root).map(
    (path) =>
      `${path} is retired: feature mock behavior belongs under src/dev-runtime/features`,
  );
  const appBootstrapPath = join(root, "src/app/bootstrap.tsx");

  if (existsSync(appBootstrapPath)) {
    const bootstrapText = readFileSync(appBootstrapPath, "utf8");
    const loadsDevelopmentRuntime =
      /\bimport\s*\(\s*["']\.\.\/dev-runtime\//.test(bootstrapText);
    const hasDevelopmentGate =
      /if\s*\(\s*!import\.meta\.env\.DEV\s*\)\s*return\s*;/.test(
        bootstrapText,
      );

    if (loadsDevelopmentRuntime && !hasDevelopmentGate) {
      violations.push(
  ...
  for (const file of collectSourceFiles(root)) {
    ...
    for (const specifier of getImportSpecifiers(text)) {
      if (
        ...
        fromLayer !== "platform" &&
        fromLayer !== "dev-runtime" &&
        fromLayer !== "test-support"
      ) {
        violations.push(
          `${relFile} imports ${specifier} outside src/platform: production desktop APIs must use the platform owner`,
        );
      }
      ...
      if (toLayer === "test-support" && fromLayer !== "test-support") {
        violations.push(
          `${relFile} imports ${specifier} across forbidden boundary (${fromLayer} -> test-support): production code must not depend on test support`,
        );
      }
      ...
      if (
        ...
        fromLayer !== "dev-runtime" &&
        fromLayer !== "test-support" &&
        normalizedRelFile !== "src/app/bootstrap.tsx"
      ) {
        violations.push(
          `${relFile} imports ${specifier} across forbidden boundary (${fromLayer} -> dev-runtime): only app bootstrap and test support may load the development runtime`,
        );
      }
