...
const repositoryPolicies = [
  [hasUnreferencedSettingsHelperComponent, "remove unreferenced settings helper component"],
  [hasUnreferencedHookModule, "remove unreferenced hook module"],
  [hasUnreferencedSourceHelper, "remove unreferenced source helper"],
  [hasStaleNotificationPreferenceSyncWrapper, "remove stale notification preference sync wrapper"],
  ...
  [hasFrontendFileUrlResumeImport, "move structured resume import file reads to backend"],
  [hasProductionExplicitAnySuppression, "remove production explicit-any suppression"],
  [hasProductionTypeErrorSuppression, "remove production TypeScript error suppression"],
  [hasProductionHookDependencySuppression, "remove production hook dependency suppression"],
  [hasProductionReactRefreshSuppression, "remove production react-refresh suppression"],
  [hasProductionSourceGlyphMarkers, "replace production source emoji markers"],
  [hasBackendScoringReasonGlyphMarkers, "replace backend scoring reason glyph markers"],
  [hasNotificationScoringReasonGlyphMarkers, "replace notification scoring reason glyph markers"],
  ...
  [hasUnverifiedPreMigrationBackup, "verify pre-migration SQLite backups before migration"],
  ...
  [hasNotificationWebhookSaveWithoutValidation, "validate notification webhook settings before saving"],
  [hasStaleSettingsPartialSaveMessage, "separate config save failures from credential save failures"],
  [hasStaleNotificationWebhookDocs, "document all notification webhook provider hosts"],
  [hasStaleWebhookSecurityDocMarkers, "replace webhook security doc stale markers"],
  [hasStaleCommandExecutionSecurityDocMarkers, "replace command execution security doc stale markers"],
  [hasStaleUrlValidationSecurityDocMarkers, "sync URL validation security doc markers"],
  ...
  [hasStaleCredentialArchitectureComments, "sync credential architecture comments"],
  [hasStaleNotificationPreferenceDocs, "sync notification preference docs with backend shape"],
  [hasUnownedStorybookAddon, "remove Storybook addon without package ownership"],
  [hasRedundantDirectPlaywrightDependency, "remove redundant direct Playwright dependency"],
  [hasDirectPlaywrightE2eScript, "route E2E scripts through Playwright wrapper"],
  [hasRedundantDomPurifyTypesDependency, "remove redundant DOMPurify stub types dependency"],
  [hasTailwindPostcssPlugin, "use Tailwind Vite plugin instead of PostCSS plugin"],
  [hasUnreferencedDocsImage, "remove unreferenced docs image"],
  [hasDuplicateDocsScreenshotCapture, "remove duplicate docs screenshot capture"],
  ...
  [hasBookmarkletTokenIpcExposure, "keep bookmarklet token out of renderer IPC"],
  [hasApplicationProfileResumePathExposure, "keep application resume paths out of renderer IPC"],
  [hasApplicationAssistAutomaticResumeUpload, "keep Application Assist resume attachment manual"],
  [hasApplicationAssistUntrustedFormTarget, "validate Application Assist target before profile load"],
  [hasAutomationScreenshotPathIpcExposure, "keep automation screenshot paths out of renderer IPC"],
  [hasRawAnswerHistoryIpcExposure, "keep raw screening answer history out of renderer IPC"],
  [hasAnswerHistoryRendererInvoke, "avoid renderer answer-history IPC"],
  [hasNonSettingsFullApplicationProfileInvoke, "use minimized application profile IPC"],
  [hasDashboardFullConfigInvoke, "use Dashboard preferences IPC"],
  [hasRawJobImportUrlAfterPreview, "use canonical import preview URL"],
  ...
  [hasStaleProfilePreviewIpcMock, "sync minimized profile mock command handlers"],
  [hasStaleFeedbackMockHandlers, "sync feedback mock command handlers"],
  [hasStaleFeedbackSystemInfoArchitecture, "sync feedback system-info architecture field"],
  [hasStaleResumeOptimizerMockHandlers, "sync resume optimizer mock command handlers"],
  [hasStaleAtsKeywordMatchFrontendShape, "sync ATS keyword match frontend shape"],
  ...
  [hasStaleSalaryBenchmarkFrontendShape, "sync salary benchmark frontend shape"],
  [hasStaleInterviewFollowupFrontendShape, "sync interview follow-up frontend shape"],
  [hasStaleResumeMatchSubscoreDisplay, "render resume match sub-scores from backend fractions"],
  [hasStaleResumeE2eMatchSeed, "sync resume E2E match seeds with backend fraction shape"],
];
