...
export function collectWorkflowPinnedNpmViolations(root, npmVersion) {
  ...
  if (!existsSync(repoPath(root, installerPath))) {
    violations.push(`${installerPath} is required so CI can activate package.json npm@${npmVersion}`);
    return violations;
  }
  ...
  for (const path of workflowFiles(root)) {
    ...
    lines.forEach((line, index) => {
      ...
      if (/(^|\s)npm\s+(?:audit|ci|run|test)\b/.test(line) && !sawPinnedNpmInstall) {
        violations.push(
          `${path}:${index + 1} npm commands after setup-node must first run \`node ${installerPath}\` so CI uses package.json npm@${npmVersion}`,
        );
      }
    });
  }
