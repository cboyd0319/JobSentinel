  ...
  const criteria = [
    ...
    criterion(
      ...
      hasAll(inputs.readme, [
        ...
        "release publication waits for final local gates",
        "user-confirmed push/publish",
        "screenshot, and whole-UI evidence have been recorded locally",
        "Final gates must",
        "be rerun from the latest commit before publication any time local changes land",
        "currently published downloads are older fallback assets until `v2.9.0`",
        "Source metadata | Local package, Tauri, and Cargo metadata are staged for `2.9.0`",
        "Published downloads | Older fallback assets remain on GitHub Releases until `2.9.0`",
