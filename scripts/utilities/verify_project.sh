...
# Check if ATS scanner is accessible
if python3 -c "from utils.ats_scanner import ATSScanner" 2>/dev/null; then
    success "ATS scanner module loads successfully"
else
    warning "ATS scanner may be missing dependencies (install with pip install -r requirements.txt)"
fi
...
echo "  3. Test Slack wizard: python scripts/slack_bootstrap.py"
echo "  4. Test ATS scanner: python -m utils.ats_scanner <resume.pdf>"
