...
test("checkRepoBloat grandfathers known oversized files without allowing growth", () => {
  withGitFixture((root) => {
    writeFixtureFile(root, "package.json", "{}\n");
    writeFixtureFile(root, "scripts/check-repo-bloat.test.mjs", lineFixture(4562));
...
test("checkRepoBloat rejects growth in grandfathered oversized files", () => {
  withGitFixture((root) => {
    writeFixtureFile(root, "package.json", "{}\n");
    writeFixtureFile(root, "scripts/check-repo-bloat.test.mjs", lineFixture(4563));
    ...
    assert.ok(
      violations.includes(
        "split legacy oversized tracked file before growing it: scripts/check-repo-bloat.test.mjs has 4563 lines (budget 4562, target 1200)",
      ),
      violations.join("\n"),
    );
  });
