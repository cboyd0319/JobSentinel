...
test("checkRepoBloat grandfathers known oversized files without allowing growth", () => {
  withGitFixture((root) => {
    writeFixtureFile(root, "package.json", "{}\n");
    writeFixtureFile(root, "scripts/check-repo-bloat.test.mjs", lineFixture(8550));
...
test("checkRepoBloat rejects growth in grandfathered oversized files", () => {
  withGitFixture((root) => {
    writeFixtureFile(root, "package.json", "{}\n");
    writeFixtureFile(root, "scripts/check-repo-bloat.test.mjs", lineFixture(8551));
    ...
    assert.ok(
      violations.includes(
        "split legacy oversized tracked file before growing it: scripts/check-repo-bloat.test.mjs has 8551 lines (budget 8550, target 1200)",
      ),
      violations.join("\n"),
    );
  });
