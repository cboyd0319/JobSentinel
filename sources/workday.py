...
def scrape(board_url: str, fetch_descriptions: bool = True):
    ...
    try:
        ...
        if "workday" in content.lower() and "jobs" in content.lower():
                    ...
                    try:
                        full_description = asyncio.run(
                            fetch_job_description(
                                job_url, '[data-automation-id="jobPostingDescription"]'
                            )
                        )
                        if full_description:
                        ...
                        logger.warning(
                            f"Failed to fetch description for {job_title}: {e}"
                        )

                job_hash = create_job_hash(
                    company_name, job_title, job_description[:250]
                )
            ...
            logger.warning(
                f"No recognizable job data found for Workday board: {board_url}"
            )
