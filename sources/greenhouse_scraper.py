...
class GreenhouseScraper(JobBoardScraper):
	...
	def __init__(self):
		super().__init__(
			name="Greenhouse", base_domains=["greenhouse.io", "boards.greenhouse.io"]
		)
	...
	async def scrape(
		...
		responses = await asyncio.gather(
			*[fetch_url(url) for url in api_urls], return_exceptions=True
		)
			...
			try:
				if response.status_code == 200:
					...
					if (
						...
						and "content" in data
						and "<html" in str(data.get("content", ""))
					):
					...
					if jobs_data and len(jobs_data) > 0:
						logger.info(f"[OK] Greenhouse API succeeded: {api_url}")
						break
		...
		if not jobs_data and "job" in board_url.lower():
			...
			if job_id_match:
				...
				if individual_job:
					...
					logger.info(f"[OK] Individual job API succeeded for {job_id}")
	...
	def _extract_greenhouse_fields(self, job: dict) -> dict:
		"""Extract optional Greenhouse job fields safely."""
		fields = {}
		...
		fields.update({
			...
			"department": dept[0].get("name") if dept else None,
			"remote": job.get("workplace_type") == "remote",
			"office": location.get("name"),
		})
