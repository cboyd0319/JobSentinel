...
class PlaywrightScraper(JobBoardScraper, APIDiscoveryMixin):
                ...
                for api_info in discovered_apis:
                    try:
                        ...
                        if data and self.contains_job_data(data):
                            working_apis.append({"url": api_info["url"], "data": data})
                            logger.info(f"[OK] Working API found: {api_info['url']}")
                    except Exception as e:  # nosec B110
                        logger.debug(f"API test failed for {api_info['url']}: {e}")
                if working_apis:
                    return self._extract_from_discovered_apis(working_apis, board_url)
            except Exception as e:  # nosec B110
    ...
    async def _extract_google_jobs(self, page, company_name: str) -> list[dict]:
        ...
        except BaseException:  # noqa: S110 - selector wait timeout is expected
            pass
        for selector in selectors:
            try:
                elements = await page.query_selector_all(selector)
                ...
                for element in elements[:20]:
                    try:
                        ...
                        if not link:
                            ...
                            if anchor:
                                link = await anchor.get_attribute("href")
                        raw_job = {"title": title, "description": text_content}
                        if link:
