...
def demo_resume_analysis():
    ...
    with track_performance("resume_analysis", {"industry": "healthcare"}):
        analysis = service.analyze_resume_text(sample_resume, target_industry="healthcare")

    print(f"\n  Current Score: {analysis.current_score:.1f}/100")
    print(f"  Potential Score: {analysis.potential_score:.1f}/100")
    print(
        f"  Improvement Potential: {analysis.potential_score - analysis.current_score:.1f} points"
    )
...
def demo_job_intelligence():
    ...
    user_prefs = {"salary_min": 120000, "locations": ["Remote", "San Francisco"]}

    ranked_jobs = engine.rank_jobs_intelligently(sample_jobs, user_skills, user_prefs)
...
def demo_security_features():
    ...
    result, errors = validator.validate_url(
        "https://jobs.company.com/careers", allowed_schemes=["https"]
    )
...
async def demo_resilience_patterns():
    ...
    retry_config = RetryConfig(max_attempts=3, base_delay_seconds=0.5, strategy="exponential")

    circuit_config = CircuitBreakerConfig(failure_threshold=3, timeout_seconds=30)

    scraper = ResilientScraper(
        name="demo_scraper", retry_config=retry_config, circuit_config=circuit_config
    )
