...
Examples:
  ...
  %(prog)s resume.pdf --summary          # Just show summary
    ...
    parser.add_argument("--keywords", help="Comma-separated list of job keywords to analyze")

    parser.add_argument("--output", "-o", help="Output file path for detailed report")
    ...
    parser.add_argument(
        "--summary", "-s", action="store_true", help="Show only summary information"
    )
...
def print_summary(service: ATSAnalysisService, score):
    ...
    print(
        f"Issues Found: {summary['total_issues']} (Critical: {summary['critical_issues_count']}, High: {summary['high_priority_issues_count']})"
    )
...
def print_detailed_results(score):
    ...
    if score.keyword_matches:
        ...
        for match in top_matches:
            print(
                f"  • {match.keyword} (Relevance: {match.relevance_score:.2f}, Count: {match.matches})"
            )
