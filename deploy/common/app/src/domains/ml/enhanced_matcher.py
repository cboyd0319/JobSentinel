...
class EnhancedMatcher:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Enhanced matcher initialized with tiers: {[t.value for t in self._available_tiers]}"
        )
    ...
    def _match_secondary(
        self, resume: str, job_desc: str, skills: list[str]
    ) -> EnhancedMatchResult:
    ...
    def _match_fallback(self, resume: str, job_desc: str, skills: list[str]) -> EnhancedMatchResult:
        ...
        try:
            ...
            return EnhancedMatchResult(
                ...
                metadata={
                    "method": "TF-IDF",
                    "features": len(feature_names) if "feature_names" in locals() else 0,
                },
            )
    ...
    def match_resume_to_job(
        self, resume_text: str, job_description: str, required_skills: list[str] | None = None
    ) -> EnhancedMatchResult:
        """
        ...
        for tier in self._available_tiers:
            try:
                if tier == MatcherTier.PRIMARY:
                    ...
                    logger.info(
                        f"Match completed with PRIMARY tier in {result.performance_ms:.1f}ms"
                    )
                    return result
                    ...
                    logger.info(
                        f"Match completed with SECONDARY tier in {result.performance_ms:.1f}ms"
                    )
                    return result
                    ...
                    logger.info(
                        f"Match completed with FALLBACK tier in {result.performance_ms:.1f}ms"
                    )
                    return result
