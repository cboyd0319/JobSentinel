...
class SystemDiagnostic:
        ...
        for group_name, packages in optional_groups.items():
            ...
            self.results.append(
                DiagnosticResult(
                    ...
                    message=message,
                    fix=(
                        f"Install with: pip install -e .[{group_name.lower().replace(' ', '_')}]"
                        if status != "pass"
                        else ""
                    ),
                )
    ...
    def print_results(self, verbose: bool = False) -> None:
        ...
        print("=" * 70)
        print(
            f"Summary: {len(passed)} passed, {len(failed)} failed, {len(warnings)} warnings, {len(skipped)} skipped"
        )
        print("=" * 70 + "\n")
        ...
        if failed:
            ...
            print(
                "⚠️  System is OK but has warnings. JobSentinel will work but may have limitations."
            )
        else:
