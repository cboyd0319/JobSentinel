...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--channel", default=DEFAULT_CHANNEL, help="Preferred channel name to create/select"
    )
    ...
    p.add_argument(
        ...
        dest="test_only",
        action="store_true",
        help="Only test existing webhook; do not modify .env",
    )
    p.add_argument(
        "--non-interactive", action="store_true", help="Fail instead of prompting when info missing"
    )
    p.add_argument(
        "--print-manifest",
        action="store_true",
        help="Print manifest path (create if missing) and exit",
    )
    p.add_argument(
        "--force",
        action="store_true",
        help="Overwrite existing SLACK_WEBHOOK_URL without prompting",
    )
        ...
        "blocks": [
            {
                ...
                "text": {
                    "type": "mrkdwn",
                    "text": "*Slack setup complete.* Job alerts will appear here.",
                },
            },
            {
                "type": "context",
...
def wizard(channel: str, quiet: bool = False) -> str:
    if not quiet:
        console.print(
            Panel(
                "Slack Setup Wizard — minimal clicks. You'll copy a webhook URL from Slack.",
    ...
    has_workspace = (
        Confirm.ask("Do you already have a Slack workspace?", default=False) if not quiet else True
    )
    ...
    if not quiet:
        ...
        console.print(
            f"Copy contents of {MANIFEST_PATH} into the manifest field, then create the app."
        )
        input("Press ENTER once the app is created...")
        console.print("\n[bold cyan]Step 3:[/bold cyan] Enable incoming webhooks & create one.")
        console.print(
            "In the app settings, go to 'Incoming Webhooks' -> toggle ON -> 'Add New Webhook to Workspace'."
        )
    ...
    while True:
        ...
        if validate_webhook(url):
            return url
        console.print(
            "[yellow]That does not look like a valid Slack webhook URL. Try again.[/yellow]"
        )
...
def main() -> int:
    ...
    if not args.quiet:
        console.print(
            Panel(
                "Slack setup complete. You will start receiving job alerts here when matches are found.",
                border_style="green",
            )
        )
    return 0
