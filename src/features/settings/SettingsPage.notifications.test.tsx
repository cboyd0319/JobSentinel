...
describe("Settings — notification safeguards", () => {
  ...
  it("uses plain notification connection wording instead of service-internal jargon", async () => {
    ...
    expect(
      screen.getByPlaceholderText(/Slack connection link/i),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByPlaceholderText(/Telegram setup code/i),
    ).toBeInTheDocument();
    expect(screen.getByText("Telegram destination")).toBeInTheDocument();
    expect(
      screen.getByText(/Optional Telegram alert setup/i),
    ).toBeInTheDocument();
  ...
  it("keeps Telegram bot details behind the optional chat-alert path", async () => {
    ...
    expect(screen.getAllByText(/Optional chat alert/i).length).toBeGreaterThan(
      0,
    );
    expect(
      screen.getByText(
    ...
    expect(
      screen.queryByPlaceholderText("Paste Telegram setup code"),
    ).not.toBeInTheDocument();
    expect(screen.queryByText("Telegram destination")).not.toBeInTheDocument();

    await user.click(
      screen.getByRole("checkbox", { name: "Enable Telegram alerts" }),
    );

    expect(
      screen.getByPlaceholderText("Paste Telegram setup code"),
    ).toBeInTheDocument();
    expect(screen.getByText("Telegram destination")).toBeInTheDocument();
  ...
  it("presents desktop and email alerts before optional chat alerts", async () => {
    ...
    expect(screen.getByText(/Start with desktop alerts/i)).toBeInTheDocument();
    expect(
      screen.getByRole("checkbox", { name: "Enable desktop alerts" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Email and chat alerts are sent through the service you choose/i,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Resume text, private notes, application history, and local match reasons stay in JobSentinel/i,
      ),
    ).toBeInTheDocument();
