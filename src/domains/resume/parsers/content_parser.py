...
class ResumeContentParser:
    ...
    SECTION_PATTERNS = {
        ...
        SectionType.PROJECTS: [r"(?i)^projects?", r"(?i)^portfolio", r"(?i)^selected\s+projects"],
        ...
        SectionType.PUBLICATIONS: [r"(?i)^publications?", r"(?i)^research", r"(?i)^papers?"],
    ...
    def _extract_contact_info(self, text: str) -> dict[str, str]:
        ...
        for line in lines[:5]:  # Check first 5 lines
            if (
                not re.search(email_pattern, line)
                and not re.search(phone_pattern, line)
                and len(line.split()) <= 4  # Names typically 2-4 words
