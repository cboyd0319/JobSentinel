...
from .models import (
    ResumeContent, ResumeAnalysis, ResumeSuggestion, ResumeTemplate,
    IndustryProfile, SectionType
)
...
class ResumeEnhancementService:
    ...
    def analyze_resume_text(
        ...
        logger.info(f"Analysis complete. Current score: {current_score}, Potential: {potential_score}")
        return analysis
    ...
    def _calculate_current_score(self, resume_content: ResumeContent, content_analysis: Dict) -> float:
        ...
        total_score = min(100, score + sections_score + content_score + readability_points + contact_score)
        return round(total_score, 1)
    
    def _calculate_potential_score(self, current_score: float, suggestions: List[ResumeSuggestion]) -> float:
    ...
    def _categorize_sections(
        ...
        present_sections = set(section.section_type.value for section in resume_content.sections.values())
        
        # Missing sections
        missing_sections = list(all_sections - present_sections)
        
        # Sections with improvement suggestions are considered weak
        sections_with_suggestions = set(s.section for s in suggestions if s.section in present_sections)
        ...
        strong_sections = [
            ...
            if section not in sections_with_suggestions
        ]
    ...
    def _detect_industry(self, resume_content: ResumeContent) -> Optional[str]:
        ...
        industry_keywords = {
            "software_engineering": [
                "software", "programming", "developer", "engineer", "python", "java",
                "javascript", "react", "api", "database", "git", "agile"
            ],
            "data_science": [
                "data", "analytics", "machine learning", "python", "sql", "statistics",
                "modeling", "visualization", "pandas", "numpy", "tensorflow"
            ],
            "marketing": [
                "marketing", "campaign", "brand", "social media", "seo", "content",
                "advertising", "digital marketing", "conversion", "engagement"
            ]
        }
    ...
    def _recommend_template(self, resume_content: ResumeContent, industry: Optional[str]) -> ResumeTemplate:
        ...
        seniority_keywords = ["senior", "lead", "director", "manager", "executive", "vp", "ceo", "cto"]
