...
from ..models import ATSCompatibilityScore, ATSIssueLevel, ATSSystem
...
class ReportGenerator:
    ...
    def generate_text_report(self, score: ATSCompatibilityScore) -> str:
        ...
        for level in [ATSIssueLevel.CRITICAL, ATSIssueLevel.HIGH, ATSIssueLevel.MEDIUM, ATSIssueLevel.LOW]:
        ...
        if score.keyword_matches:
            lines.append("\nTOP KEYWORD MATCHES")
            lines.append("-" * 40)
            top_matches = sorted(score.keyword_matches, key=lambda x: x.relevance_score, reverse=True)[:10]
            for match in top_matches:
                status = "✓" if match.is_skill else "○"
                lines.append(f"{status} {match.keyword} (Relevance: {match.relevance_score:.2f}, Matches: {match.matches})")
...
<body>
    ...
    def save_report(self, score: ATSCompatibilityScore, output_path: Optional[str] = None, format: str = "text") -> str:
    ...
    def _generate_issues_html(self, score: ATSCompatibilityScore) -> str:
        ...
        for level in [ATSIssueLevel.CRITICAL, ATSIssueLevel.HIGH, ATSIssueLevel.MEDIUM, ATSIssueLevel.LOW]:
            issues_at_level = score.get_issues_by_level(level)
            if issues_at_level:
                html.append(f'<h3 class="issue-{level.value}">{level.value.upper()} Issues ({len(issues_at_level)})</h3>')
                html.append('<ul>')
    ...
    def _generate_keywords_html(self, score: ATSCompatibilityScore) -> str:
        ...
        top_matches = sorted(score.keyword_matches, key=lambda x: x.relevance_score, reverse=True)[:15]
        for match in top_matches:
