...
class ATSAnalysisService:
    ...
    def analyze_resume(
        ...
        logger.info(f"Resume parsed successfully. Word count: {metadata.get('word_count', 'unknown')}")
    ...
    def get_analysis_summary(self, score: ATSCompatibilityScore) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_recommendation": score.priority_recommendations[0] if score.priority_recommendations else None,
...
def analyze_resume(resume_path: str, job_keywords: List[str] = None) -> ATSCompatibilityScore:
    ...
    """
    logger.warning(
        "analyze_resume() is deprecated. "
        "Use ATSAnalysisService.analyze_resume() for new code."
    )
