...
from ..models import ATSCompatibilityScore, ATSIssue, ATSIssueLevel, ATSSystem, KeywordMatch
...
class CompatibilityScorer:
    ...
    def _calculate_keyword_score(self, keyword_matches: List[KeywordMatch], job_keywords: List[str] = None) -> float:
    ...
    def _calculate_system_scores(self, issues: List[ATSIssue], overall_score: float) -> Dict[ATSSystem, float]:
        ...
        for ats_system in ATSSystem:
            ...
            final_score = max(0, min(100, base_score + (overall_score - 70) * 0.3 - total_deduction))
    ...
    def _generate_quick_wins(self, issues: List[ATSIssue], keyword_matches: List[KeywordMatch]) -> List[str]:
