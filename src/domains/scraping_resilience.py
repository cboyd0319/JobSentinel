...
class ScraperHealthMonitor:
    ...
    def __init__(self):
        self.health_data: dict[str, dict[str, Any]] = defaultdict(lambda: {
            "successes": 0,
            "failures": 0,
    ...
    def get_health_status(self, scraper_name: str) -> HealthStatus:
        ...
        success_rate = (
            data["successes"] / total_attempts if total_attempts > 0 else 0.0
        )
        
        avg_latency = (
            sum(data["latencies"]) / len(data["latencies"])
            if data["latencies"] else 0.0
        )
    ...
    def get_all_health_statuses(self) -> list[HealthStatus]:
        ...
        return [
            self.get_health_status(name)
            for name in self.health_data.keys()
        ]
    
    def get_unhealthy_scrapers(self) -> list[HealthStatus]:
        """Get list of unhealthy scrapers."""
        return [
            status for status in self.get_all_health_statuses()
