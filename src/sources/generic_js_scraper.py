...
class GenericJSScraper(JobBoardScraper, APIDiscoveryMixin):
    ...
    async def scrape(self, board_url: str, fetch_descriptions: bool = True) -> list[dict]:
        ...
        for strategy_name, strategy_func in strategies:
            try:
                ...
                if jobs:
                    logger.info(f"✅ {strategy_name} succeeded: found {len(jobs)} jobs")
                    return jobs
    ...
    async def _scrape_via_dom(self, board_url: str, fetch_descriptions: bool) -> list[dict]:
        """
                    ...
                    try:
                        ...
                        if elements:
                            ...
                            for element in elements[:50]:  # Limit to 50 jobs
                                job_data = await self._extract_job_from_element(element, board_url)
                                if job_data:
    ...
    async def _scrape_via_json(self, board_url: str, fetch_descriptions: bool) -> list[dict]:
        """
    ...
    async def _extract_job_from_element(self, element: Any, board_url: str) -> dict | None:
        """
        ...
        try:
            # Extract title
            title_selectors = ["h2", "h3", '[class*="title"]', '[class*="position"]']
            title = None
            ...
            # Extract location
            location_selectors = ['[class*="location"]', '[class*="office"]', "[data-location]"]
            ...
            job["hash"] = create_job_hash(job["company"], job["title"], job["description"])
    ...
    def _extract_from_api_responses(self, api_requests: list[dict], board_url: str) -> list[dict]:
        """Extract jobs from discovered API responses."""
        jobs = []
    ...
    def _normalize_job_object(self, obj: dict, company: str, board_url: str) -> dict | None:
        ...
        try:
            ...
            # Extract location
            location = (
                obj.get("location") or obj.get("office") or obj.get("city") or "Not specified"
            )
            ...
            # Extract description
            description = obj.get("description") or obj.get("summary") or obj.get("details") or ""
    ...
    def _looks_like_job_data(data: Any) -> bool:
        ...
        job_keywords = ["job", "position", "posting", "opening", "role", "career", "vacancy"]
