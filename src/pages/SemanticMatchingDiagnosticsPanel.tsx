...
function DiagnosticsSummary({
  ...
  const highlightedModels = diagnostics.models.filter(
    (model) => model.required_for_qwen3_runtime || model.role.includes("Legacy"),
  );
        ...
        <div>
          <div className="mb-2 flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs text-surface-500 dark:text-surface-400">
              {diagnostics.build_enabled ? "Model build enabled" : "Model build not enabled"}
            </span>
          </div>
          ...
          >
            ...
            <div className="text-left sm:text-right">
              ...
              <div className="text-xs text-surface-500 dark:text-surface-400">
                {model.required_files_present}/{model.required_files} files present
              </div>
            </div>
          </div>
        ))}
        ...
        <ul className="space-y-1.5 text-xs text-surface-600 dark:text-surface-400">
          {diagnostics.eval_contract.map((item) => (
            <li key={item} className="flex gap-2">
              <SettingsSymbol icon="check" className="mt-0.5 h-3.5 w-3.5 flex-shrink-0" />
              <span>{item}</span>
            </li>
          ))}
        </ul>
