...
describe("Settings toast-triggered form state", () => {
  ...
  it("keeps unsaved detected location after success toast renders", async () => {
    ...
    mockInvoke.mockImplementation(async (cmd: string) => {
      ...
      if (cmd === "get_system_info") {
        return { app_version: "test", platform: "test", os_version: "test", architecture: "test" };
      }
  ...
  it("adds company preferences when a saved config has no company arrays", async () => {
    ...
    delete legacyConfig.company_whitelist;
    delete legacyConfig.company_blacklist;
    ...
    expect(
      await screen.findByRole("button", { name: "Remove Verification Preferred Co" }),
    ).toBeInTheDocument();
    ...
    expect(
      await screen.findByRole("button", { name: "Remove Verification Avoid Co" }),
    ).toBeInTheDocument();
  });
  ...
  it("shows desktop alert options when saved config has no desktop alert object", async () => {
    ...
    expect(screen.getByRole("checkbox", { name: "Enable desktop alerts" }))
      .toBeChecked();
    expect(screen.getByRole("checkbox", { name: "Play sound" }))
      .not.toBeChecked();
    expect(
