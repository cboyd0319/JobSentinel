...
function disabledDiagnostics() {
  return {
    ...
    user_action: "Advanced local model diagnostics appear in embedded-ML builds.",
  };
}
...
function readyDiagnostics() {
  return {
    ...
    eval_contract: ["Generated advice must stay separate from real job evidence."],
    user_action: null,
  };
}
...
describe("SemanticMatchingDiagnosticsPanel", () => {
  ...
  it("shows the local fallback when embedded ML is not enabled", async () => {
    ...
    expect(await screen.findByText("Using built-in local matching rules")).toBeInTheDocument();
    expect(screen.getByText("Built-in local matching rules")).toBeInTheDocument();
    expect(screen.getByText(/No resume or job text leaves this device/)).toBeInTheDocument();
    expect(screen.getByText(/Advanced local model details appear/)).toBeInTheDocument();
  });
  ...
  it("shows Qwen3 model cache details and refreshes on request", async () => {
    ...
    expect(await screen.findByText("Advanced local matching is ready")).toBeInTheDocument();
