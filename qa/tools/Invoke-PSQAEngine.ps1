...
function Get-PSFile {
    ...
    $files = Get-ChildItem -Path -Path $Path -Recurse -File | Where-Object {
...
function Set-SingleFix {
    ...
    switch ($Issue.RuleName) {
        ...
        'PSAvoidUsingWriteHost' {
            # Replace Write-Output with Write-Output for better practices
            $Content = $Content -replace '\bWrite-Output\b', 'Write-Output'
            return $Content
        }
        'PSAvoidUsingPositionalParameters' {
            # Fix common positional parameter usage
            $fixes = @{
                'Set-Variable\s+([^\s]+)\s+([^\s]+)' = 'Set-Variable -Name -Name -Value $1 -Value $2'
                'Join-Path\s+([^\s]+)\s+([^\s]+)' = 'Join-Path -Path -Path -ChildPath $1 -ChildPath $2'
                'Get-ChildItem\s+([^\s]+)' = 'Get-ChildItem -Path -Path $1'
                'Test-Path\s+([^\s]+)' = 'Test-Path -Path -Path $1'
                'Remove-Item\s+([^\s]+)' = 'Remove-Item -Path -Path $1'
                'New-Item\s+([^\s]+)' = 'New-Item -Path -Path $1'
            }
...
<#
.SYNOPSIS
    $functionName [OutputType([object])]
function
.DESCRIPTION
...
"@
        ...
        'PSProvideCommentHelp' {
            # Add comment-based help for functions that don't have it
            if ($Issue.Message -match "cmdlet '([^']+)' does not have a help comment") {
                $functionName = $Matches[1]
                # Clean up function name (remove global: prefix if present)
                $cleanFunctionName = $functionName -replace '^global:', ''

                $helpBlock = @"
<#
.SYNOPSIS
    $cleanFunctionName function provides specific functionality.
.DESCRIPTION
    This function performs operations related to $cleanFunctionName.
.EXAMPLE
    $cleanFunctionName
    Executes the $cleanFunctionName function.
#>
"@
        ...
        'PSReviewUnusedParameter' {
            # Remove unused parameters more aggressively
            if ($Issue.Message -match "parameter '([^']+)' has been declared but not used") {
                $paramName = $Matches[1]

                # If it's the __error__ parameter, just remove the entire parameter line
                if ($paramName -eq '__error__') {
                    $lines = $Content -split "`n"
                    $fixedLines = @()

                    foreach ($line in $lines) {
                        # Skip lines that declare the __error__ parameter
                        if ($line -match '^\s*\[Parameter.*\]\s*$' -and
                            ($lines[$lines.IndexOf($line) + 1] -match '^\s*\$__error__')) {
                            # Skip the [Parameter] line and the next line with $__error__
                            $fixedLines += $line  # Keep the Parameter attribute for now
                            continue
                        }
                        if ($line -match '^\s*\$__error__.*') {
...
# Execute the main function with the script parameters
Invoke-PSQAEngine -Path $Path -Mode $Mode -Runtime $Runtime -DryRun:$DryRun -ConfigPath $ConfigPath -OutputFormat $OutputFormat -TraceId $TraceId -BackupEnabled:$BackupEnabled
