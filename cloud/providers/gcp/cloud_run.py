...
async def build_and_push_image(logger, project_root: Path, project_id: str, region: str, artifact_repo: str) -> str:
    ...
    image_tag = (
        f"{region}-docker.pkg.dev/{project_id}/{artifact_repo}/"
        "job-scraper:latest"
    )
    ...
    try:
        ...
        await run_command(
            ...
            retries=3, delay=10 # Add retries for Cloud Build
        )
        ...
        await run_command(
            ...
            logger=logger,
            stream_output=True,
            retries=3, delay=10 # Add retries for Cloud Build (fallback)
        )
    return image_tag
...
async def create_or_update_job(logger, project_id: str, region: str, job_name: str, image_uri: str, runtime_sa: str, job_mode: str, storage_bucket: str, connector_name: str, prefs_secret_name: str, slack_webhook_secret_name: str) -> None:
    ...
    common_args = [
        ...
        "--vpc-connector", connector_name,  # Private networking
