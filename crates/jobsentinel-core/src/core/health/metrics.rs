...
pub async fn get_all_scraper_health(db: &Database) -> Result<Vec<ScraperHealthMetrics>> {
    let rows = sqlx::query_as::<_, ScraperHealthRow>(
        r#"
        SELECT scraper_name, display_name, is_enabled, requires_auth, scraper_type,
               selector_health, rate_limit_per_hour,
               CAST(success_rate_24h AS REAL) AS success_rate_24h,
               CAST(avg_duration_ms AS INTEGER) AS avg_duration_ms,
