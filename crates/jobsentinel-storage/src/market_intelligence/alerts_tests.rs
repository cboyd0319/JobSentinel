...
#[path = "alerts_tests/async_entity_tests.rs"]
mod async_entity_tests;
...
mod async_tests {
    ...
    pub(super) async fn load_all_alerts(pool: &SqlitePool, limit: usize) -> Vec<MarketAlert> {
        let rows = sqlx::query(
            r#"
            SELECT
    ...
    async fn test_row_to_alert_preserves_sqlite_created_at() {
        ...
        let alerts = load_all_alerts(&pool, 1).await;
        assert_eq!(alerts.len(), 1);
        assert_eq!(
    ...
    async fn test_mark_all_read_with_data() {
        ...
        let all = load_all_alerts(&pool, 10).await;
        assert_eq!(all.len(), 3);
