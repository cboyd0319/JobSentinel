...
fn require_regular_file(path: &Path) -> Result<(), sqlx::Error> {
...
fn move_database_bundle(source: &Path, destination: &Path) -> Result<(), sqlx::Error> {
...
fn remove_database_bundle(path: &Path) -> Result<(), sqlx::Error> {
...
fn remove_if_exists(path: &Path) -> Result<(), sqlx::Error> {
...
fn previous_path(database_path: &Path, restore_id: &str) -> PathBuf {
    sibling_path(database_path, &format!(".restore-{restore_id}.previous"))
}

fn rollback_path(database_path: &Path, restore_id: &str) -> Result<PathBuf, sqlx::Error> {
    ...
    Ok(backup_dir.join(format!("restore_rollback_{restore_id}.db")))
}
