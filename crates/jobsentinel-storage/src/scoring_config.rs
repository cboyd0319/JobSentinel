...
use jobsentinel_domain::ScoringConfig;
use sqlx::SqlitePool;

/// Load scoring configuration from database
pub(crate) async fn load_scoring_config(pool: &SqlitePool) -> Result<ScoringConfig, String> {
...
pub(crate) async fn save_scoring_config(
    pool: &SqlitePool,
...
pub(crate) async fn reset_scoring_config(pool: &SqlitePool) -> Result<(), String> {
