...
impl SimplyHiredScraper {
    ...
    async fn fetch_jobs_from_url(&self, url: String) -> ScraperResult {
        let response = send_external_http_text_with_retry(Self::build_request(&url))
            .await
            .map_err(|error| ScraperError::from_external("simplyhired", error))?;
    ...
    async fn fetch_jobs_from_test_url(&self, url: String) -> ScraperResult {
        let response = send_test_http_text_with_retry(Self::build_request(&url))
            .await
            .map_err(|error| ScraperError::from_external("simplyhired", error))?;
    ...
    async fn parse_response(&self, url: String, response: ExternalTextResponse) -> ScraperResult {
    ...
    fn parse_item(&self, item: &RssItem) -> Option<Job> {
        ...
        Some(Job {
            ...
            ..Job::newly_discovered(title, company, url, location, "simplyhired", Utc::now())
        })
    }
