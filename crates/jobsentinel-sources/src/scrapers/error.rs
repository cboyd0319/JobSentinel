...
impl fmt::Display for ScraperError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Timeout { timeout_secs } => write!(f, "Request timeout after {timeout_secs}s"),
            Self::Network => write!(f, "Network request failed"),
...
impl ScraperError {
    pub fn from_external(
        ...
        error: jobsentinel_network::ExternalFetchError,
    ) -> Self {
        match error {
            jobsentinel_network::ExternalFetchError::Timeout => Self::Timeout { timeout_secs: 30 },
            jobsentinel_network::ExternalFetchError::Request
            | jobsentinel_network::ExternalFetchError::Client => Self::Network,
            jobsentinel_network::ExternalFetchError::InvalidTarget(reason) => Self::InvalidUrl {
                url: "configured source".to_string(),
                reason,
            },
            jobsentinel_network::ExternalFetchError::Body(_) => Self::Generic {
                scraper: scraper.into(),
                message: "Job source response could not be read".to_string(),
            },
        }
    }

    pub fn parse_json<T>(url: &str, body: &str) -> Result<T, Self>
    where
        T: serde::de::DeserializeOwned,
    {
        serde_json::from_str(body).map_err(|source| Self::parse("JSON", url, source))
    }
