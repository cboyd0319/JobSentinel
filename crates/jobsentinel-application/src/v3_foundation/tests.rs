//! Groups focused application-foundation behavior and evidence consistency tests.

use super::*;

#[path = "evidence_diagnostic_tests.rs"]
mod evidence_diagnostic_tests;
#[path = "evidence_tests.rs"]
mod evidence_tests;
#[path = "military_builder_flow_tests.rs"]
mod military_builder_flow_tests;
#[path = "military_tests.rs"]
mod military_tests;
#[path = "military_transition_tests.rs"]
mod military_transition_tests;
#[path = "opportunity_case_evidence_wall_tests.rs"]
mod opportunity_case_evidence_wall_tests;
#[path = "saved_match_debugger_tests.rs"]
mod saved_match_debugger_tests;
#[path = "saved_match_military_transition_tests.rs"]
mod saved_match_military_transition_tests;
#[path = "saved_match_packets_tests.rs"]
mod saved_match_packets_tests;
