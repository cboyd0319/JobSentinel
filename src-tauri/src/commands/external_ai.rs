...
use crate::core::config::{ExternalAiConfig, ExternalAiProvider};
...
pub async fn send_external_ai_request(
    ...
    let validated = validate_external_ai_command_request(&request, &external_ai)?;
    let api_key = retrieve_provider_key(validated.provider, state.credentials.as_ref()).await?;
    let text = send_provider_request(&validated, api_key.as_str()).await?;

    Ok(ExternalAiCommandResponse { text })
}

fn validate_external_ai_command_request(
    request: &ExternalAiCommandRequest,
...
async fn retrieve_provider_key(
    ...
    let key = credential_key_for_provider(provider)
...
mod tests {
    ...
    fn config_for(provider: ExternalAiProvider) -> ExternalAiConfig {
        ExternalAiConfig {
            ...
            redaction: crate::core::config::ExternalAiRedactionConfig { enabled: true },
    ...
    fn validates_reviewed_public_job_summary() {
        ...
        let validated =
            validate_external_ai_command_request(&request, &config_for(request.provider))
                .expect("request should validate");

        assert_eq!(validated.provider, ExternalAiProvider::OpenAi);
        assert_eq!(validated.model, "provider-model");
        assert!(validated.prompt.contains("Reviewed public job details"));
        assert!(validated.prompt.contains("untrusted data"));
    }
    ...
    fn rejects_unreviewed_request() {
        ...
        let error = validate_external_ai_command_request(&request, &config_for(request.provider))
