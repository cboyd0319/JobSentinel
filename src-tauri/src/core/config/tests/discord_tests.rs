...
fn test_discord_enabled_with_empty_webhook_passes_validation() {
    // In v2.9+, Discord webhook is stored through CredentialService, not config.
    ...
    assert!(
        result.is_ok(),
        "Empty Discord webhook should pass validation (credential is behind CredentialService)"
    );
}
...
fn test_discord_webhook_format_not_validated_in_config() {
    ...
    assert!(
        result.is_ok(),
        "Discord webhook format not validated in config (validated by CredentialService)"
    );
}
...
fn test_discord_webhook_length_not_validated_in_config() {
    ...
    assert!(
        result.is_ok(),
        "Discord webhook length not validated in config (validated by CredentialService)"
    );
}
