...
mod tests {
    ...
    async fn test_kanban_all_status_buckets() {
        ...
        tracker.update_status(app3, ApplicationStatus::Applied).await.unwrap();
        tracker.update_status(app3, ApplicationStatus::ScreeningCall).await.unwrap();

        let app4 = tracker.create_application("job4").await.unwrap();
        tracker.update_status(app4, ApplicationStatus::Applied).await.unwrap();
        tracker.update_status(app4, ApplicationStatus::PhoneInterview).await.unwrap();

        let app5 = tracker.create_application("job5").await.unwrap();
        tracker.update_status(app5, ApplicationStatus::Applied).await.unwrap();
        tracker.update_status(app5, ApplicationStatus::TechnicalInterview).await.unwrap();

        let app6 = tracker.create_application("job6").await.unwrap();
        tracker.update_status(app6, ApplicationStatus::Applied).await.unwrap();
        tracker.update_status(app6, ApplicationStatus::OnsiteInterview).await.unwrap();

        let app7 = tracker.create_application("job7").await.unwrap();
        tracker.update_status(app7, ApplicationStatus::Applied).await.unwrap();
        tracker.update_status(app7, ApplicationStatus::OfferReceived).await.unwrap();

        let app8 = tracker.create_application("job8").await.unwrap();
        tracker.update_status(app8, ApplicationStatus::Applied).await.unwrap();
        tracker.update_status(app8, ApplicationStatus::OfferReceived).await.unwrap();
        tracker.update_status(app8, ApplicationStatus::OfferAccepted).await.unwrap();

        let app9 = tracker.create_application("job9").await.unwrap();
        tracker.update_status(app9, ApplicationStatus::Applied).await.unwrap();
        tracker.update_status(app9, ApplicationStatus::OfferReceived).await.unwrap();
        tracker.update_status(app9, ApplicationStatus::OfferRejected).await.unwrap();

        let app10 = tracker.create_application("job10").await.unwrap();
        tracker.update_status(app10, ApplicationStatus::Applied).await.unwrap();
        tracker.update_status(app10, ApplicationStatus::Rejected).await.unwrap();

        let app11 = tracker.create_application("job11").await.unwrap();
        tracker.update_status(app11, ApplicationStatus::Applied).await.unwrap();
        tracker.update_status(app11, ApplicationStatus::Ghosted).await.unwrap();

        let app12 = tracker.create_application("job12").await.unwrap();
        tracker.update_status(app12, ApplicationStatus::Applied).await.unwrap();
        tracker.update_status(app12, ApplicationStatus::Withdrawn).await.unwrap();
    ...
    async fn test_auto_reminder_for_offer_status() {
        ...
        sqlx::query!("DELETE FROM application_reminders WHERE application_id = ?", app_id)
            .execute(&pool)
            .await
            .unwrap();
