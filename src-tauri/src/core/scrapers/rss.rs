...
pub(crate) type RssParseError = Box<dyn Error + Send + Sync>;
...
pub(crate) fn parse_rss_items(xml: &str, limit: usize) -> Result<Vec<RssItem>, RssParseError> {
    if limit == 0 {
        return Ok(Vec::new());
    }
...
#[cfg(test)]
pub(crate) fn extract_xml_tag(fragment: &str, tag: &str) -> Option<String> {
