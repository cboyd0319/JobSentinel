# Resume Tailoring Notes

## Role

Company:
Title:
Source:

## Must-Haves

| Requirement | Resume evidence | State | User must confirm | Action | Do not infer |
| ----------- | --------------- | ----- | ----------------- | ------ | ------------ |
|  |  | Strong / Partial / Missing / Not true |  |  | Unsupported credential, metric, or authorization |

## Preferred or Helpful

| Requirement | Resume evidence | Unknown | Edit if truthful | Risk level |
| ----------- | --------------- | ------- | ---------------- | ---------- |
|  |  |  |  |  |

## Bullet Edits

| Current wording | Proposed wording | Evidence used | User must confirm | Keyword cap |
| --------------- | ---------------- | ------------- | ----------------- | ----------- |
|  |  |  |  |  |

## Change Log

| Change | Why it helps | Source that supports it | User must confirm | Do not add |
| ------ | ------------ | ----------------------- | ----------------- | ---------- |
|  |  | Resume / job post / user note |  | Unsupported metric, credential, or tool |

## ATS And Readability

| Check | Evidence | Next action |
| ----- | -------- | ----------- |
| Standard headings |  |  |
| Selectable text |  |  |
| Simple bullets |  |  |
| Consistent dates |  |  |
| No hidden content |  |  |

## Do Not Add

- [Unsupported term]

## Questions for the User

1. [Question]
2. [Question]
3. [Question]
