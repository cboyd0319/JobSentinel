...
[WARNING] WARNING: This aggregates scrapers from multiple sites.
...
# Skip running this integration-heavy example in automated test suite
pytest.skip("Skipping example integration test (network & external dependencies)", allow_module_level=True)
import sys
...
async def test_deduplication():
 ...
 print(f"\n[OK] Deduplication will catch {len(jobs) - len(unique_urls)} duplicate URLs")
 else:
 print(f"\n[OK] No URL duplicates detected across sources")

 if len(jobs) > len(job_keys):
 print(f"[OK] Deduplication will catch {len(jobs) - len(job_keys)} similar jobs")
...
async def main():
 ...
 print("\n[WARNING] WARNING: This uses web scraping. Some sites may:")
 ...
 print(f"\n[ERROR] JobSpy MCP server not found!")
 print("\nTo install:")
