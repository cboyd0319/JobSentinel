...
pytest.skip("Skipping example integration test (requires API key)", allow_module_level=True)
import os
import sys
...
async def test_salary_filter():
 ...
 api_key = os.environ.get("REED_API_KEY")
 if not api_key:
 print("\n[ERROR] REED_API_KEY not set. Skipping test.")
 return
...
async def test_deduplication():
 ...
 print(f"\n[OK] Deduplication will catch {len(all_jobs) - len(unique_urls)} duplicates")
 else:
 print(f"\n[OK] No duplicates detected (as expected for same API calls)")
