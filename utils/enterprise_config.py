...
class MemoryConfigurationSource(IConfigurationSource):
    ...
    def __init__(self, source: ConfigurationSource, initial_config: Optional[Dict[str, Any]] = None):
        self.source = source
...
class ConfigurationManager(IConfigurationProvider):
    ...
    def add_source(self, source: ConfigurationSource) -> 'ConfigurationManager':
        ...
        sorted_pairs = sorted(zip(self._sources, self._source_metadata), 
    ...
    def add_change_listener(self, listener: Callable[[Dict[str, Any]], None]) -> 'ConfigurationManager':
