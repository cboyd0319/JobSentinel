...
class JobSearchError(Exception):
    ...
    def log_structured(self, logger: logging.Logger, level: int = logging.ERROR) -> None:
        ...
        logger.log(level, "Structured exception occurred", extra={
...
class TransientError(JobSearchError):
    ...
    def __init__(
        ...
        default_action = action or f"Retry after {retry_after or 60} seconds with exponential backoff"
...
class ErrorContextManager:
    ...
    def __enter__(self) -> ErrorContext:
        self.logger.debug(f"Starting operation: {self.operation}", extra={
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_type is None:
            self.logger.debug(f"Completed operation: {self.operation}", extra={
                "trace_id": self.context.trace_id,
                "component": self.component
            })
        else:
