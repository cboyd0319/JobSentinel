...
from typing import Any, Dict, List, Optional, Set, Type, Callable, Protocol
from dataclasses import dataclass, field
from enum import Enum
import logging

from utils.dependency_injection import ILogger, IConfigurationProvider, inject
...
class PluginRegistry:
    ...
    def _discover_from_module_object(self, module: Any) -> int:
        ...
        for name, obj in inspect.getmembers(module, inspect.isclass):
            if (obj != IPlugin and 
                issubclass(obj, (IErrorHandlerPlugin, IErrorTypePlugin, IErrorProcessorPlugin)) and
                hasattr(obj, 'metadata')):
                
                # Create plugin descriptor
                try:
                    metadata = obj.metadata if isinstance(obj.metadata, PluginMetadata) else PluginMetadata(**obj.metadata)
    ...
    def _check_dependencies(self, descriptor: PluginDescriptor) -> bool:
        ...
        for dep in descriptor.metadata.dependencies:
            if dep not in self._plugins:
                self.logger.error(f"Plugin {descriptor.metadata.name} depends on {dep} which is not available")
                return False
            
            dep_descriptor = self._plugins[dep]
            if dep_descriptor.lifecycle not in (PluginLifecycle.LOADED, PluginLifecycle.ACTIVE):
                self.logger.error(f"Plugin {descriptor.metadata.name} depends on {dep} which is not loaded")
                return False
