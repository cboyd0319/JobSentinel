		...
		try:
			with self._lock:
				if (
					...
					and self.log_file.stat().st_size > self.rotate_max_bytes
				):
					self._rotate()
...
class AnomalyDetector:
	...
	def detect_anomalies(self, hours: int = 1) -> List[Dict[str, Any]]:
		...
		security_events = [
			...
			for e in events
			if e["event_type"]
			in ["injection_attempt", "path_traversal", "suspicious_activity"]
		]
		if security_events:
			anomalies.append(
				{
		...
		avg_count = (
			sum(server_counts.values()) / len(server_counts) if server_counts else 0
		)
...
def _log_rotation_summary(base: Path, keep: int) -> None:
	...
	if existing:
		summary = ", ".join(
			f"{p.name}({_human_size(p.stat().st_size)})" for p in sorted(existing)
		)
