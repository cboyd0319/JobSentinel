...
class JobSearchRequest(BaseModel):
    ...
    keywords: List[constr(min_length=1, max_length=100)] = Field(
        ...
        description="Search keywords (max 100 chars each)"
    )
    ...
    distance: Optional[int] = Field(
        ...
        ge=1000,
        le=200000,
        description="Search radius in meters (1km - 200km)"
    )
        ...
        for keyword in v:
            for pattern in dangerous_patterns:
                if re.search(pattern, keyword, re.IGNORECASE):
                    raise ValueError(
                        f"Dangerous pattern detected in keyword: {pattern}"
                    )
...
class ReedJobSearchRequest(BaseModel):
    ...
    minimum_salary: Optional[int] = Field(
        ...
        ge=0,
        le=1000000,
        description="Minimum salary in GBP (0-1M)"
    )
    ...
    maximum_salary: Optional[int] = Field(
        ...
        ge=0,
        le=1000000,
        description="Maximum salary in GBP (0-1M)"
    )
    ...
    results_to_take: int = Field(
        ...
        ge=1,
        le=100,
        description="Results per request (1-100)"
    )
...
class JobSpySearchRequest(BaseModel):
    ...
    results_wanted: int = Field(
        ...
        ge=1,
        le=100,
        description="Results per site (1-100)"
    )
    ...
    hours_old: int = Field(
        ...
        ge=1,
        le=720,  # 30 days max
        description="Job age in hours (1-720)"
    )
...
class MCPServerConfig(BaseModel):
    ...
    allowed_networks: List[str] = Field(
        default_factory=list,
        description="Allowed network destinations (URLs or IPs)"
    )
    ...
    max_requests_per_hour: int = Field(
        ...
        ge=1,
        le=10000,
        description="Rate limit (1-10000 req/hr)"
    )
    ...
    timeout_seconds: int = Field(
        ...
        ge=5,
        le=300,
        description="Request timeout (5-300 seconds)"
    )
    ...
    def validate_networks(cls, v):
        ...
        for network in v:
            ...
            if not (network.startswith('http://') or
                    network.startswith('https://') or
