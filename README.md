...
universal architecture, mounted launch, installed launch, visible window, and
private isolated-data smoke checks. Full public Mac readiness cannot honestly
reach 100% without Developer ID signing, notarization, stapling, and Gatekeeper
acceptance, all of which require Apple Developer Program materials.

The currently published downloads are older fallback assets until `v2.9.0`
is published. Current active work and release blockers are recorded in
[active plan status](docs/plans/active/status.md).
...
The `2.9.0` release candidate expands JobSentinel from a source-checking app
into a fuller local job-search workspace:
...
- LinkedIn-compatible Workbench model for user-opened sessions, local ledger
  ...
  without cookie, token, storage, or hidden-page capture.
- Broader job-source taxonomy covering public APIs, public employer pages,
  regional boards, restricted public boards, authenticated sources, and
  source-specific no-account limitations.
...
- Application Assist with review-first form guidance, screening-answer review,
  cover-letter checklist, and manual final submission.
- Pay Protection for listed-pay gaps, written-vs-verbal offer separation,
  commute/relocation review, deadline pressure, and local counter/decline
  starter notes.
...
Download the latest published package or installer from the
[latest download page](https://github.com/cboyd0319/JobSentinel/releases/latest).
The `2.9.0` release candidate is not published yet. Until it is published and
verified, the GitHub Releases page may still show older fallback assets.

| Platform | Download |
| -------- | --------- |
| Windows | `2.9.0` will publish MSI and setup EXE assets after final gates. Unsigned no-account assets must include `_unsigned` in the filename when signing credentials are unavailable. |
| macOS | `2.9.0` will publish a universal Mac DMG for Apple silicon and Intel Macs after final gates. Until Developer ID signing and notarization are available, the no-account DMG filename must include `_no-account_`, and macOS may require first-open Privacy & Security approval. |
...
Use only release assets with matching `.sha256` checksum files from the same
release. If a macOS release is missing the checksum or no-account label, treat
...
Read the full [job source guide](docs/features/job-sources.md) and
[job source status docs](docs/features/job-source-status.md).
...
- README, docs hub, release notes, and screenshots reflect current `2.9.0`
  behavior.
- Every current scraper/source path and LinkedIn-compatible browser-capture
  path has manual evidence.
- Every resume feature has been run against the local resume corpus and profile
  references.
- Web and Tauri UI passes cover every route, click, action, modal, toast,
  empty/loading/error state, settings panel, import flow, keyboard path, and
  narrow-width surface.
- Final local gates pass from the verified commit.
- The user confirms push, tag, hosted build, publication, and public artifact
  verification.

The `v2.9.0` release notes live in
