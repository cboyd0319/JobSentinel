...
- Resume/job fit now treats `records management` and `records-management` as
  equivalent local evidence without duplicate requirement rows for hyphenated
  wording.
- Resume/job fit now treats `case files` and `case-files` as equivalent local
...
- Resume/job fit now treats OSHA 30, OSHA30, OSHA 30 certification, and OSHA
  ...
  10 as equivalent.
- Resume/job fit now treats `high school diploma`, `high-school diploma`,
...
- Resume/job fit now treats `master's degree`, `master degree`,
  ...
  `Master of Science`, and `Master of Social Work` as equivalent local
