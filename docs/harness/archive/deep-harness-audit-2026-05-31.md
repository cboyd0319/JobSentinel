...
| P0 | Extract hardcoded harness policy data from `check-harness.mjs` | `docs/harness/manifest.json` owns required harness files, policy snippets, and README reference-source URLs; `scripts/checks/harness.mjs` orchestrates focused contract and repository-file modules. | Closed: source and snippet policy changes are reviewable as data instead of script edits. | Keep new harness policy lists in the manifest unless logic, not policy data, changes. |
| P1 | Split `check-repo-bloat.mjs` into named sensors | Closed: `scripts/checks/repo-bloat.mjs` is a thin orchestrator for product policy, repository policy, and focused modules under `scripts/harness/checks/`. | Review cost is assigned to named policy owners with focused coverage. | Split a named module only when a real ownership boundary and focused verification exist. |
...
| P1 | Add changed-test suggestion support | Closed: `npm run harness:plan -- --since origin/main` suggests adjacent Vitest tests, changed Playwright specs, script tests, Rust gates, Tauri invoke checks, and fallback unit suites when no adjacent frontend test exists. | Closed for current repo layout; new test roots must be added with focused coverage. | Update the planner and `scripts/tests/harness/harness-plan.test.mjs` when test layout changes. |
