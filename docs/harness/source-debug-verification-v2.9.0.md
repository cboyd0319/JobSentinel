...
| Restricted-source UI and import contracts | `npm run test:run -- src/shared/jobSourceDiscoveryTaxonomy.test.ts src/shared/restrictedSourceTaxonomy.test.ts src/shared/linkedinWorkbench.test.ts src/features/settings/sources/browser-import/BrowserImportButtonScript.test.ts src/components/LinkedInWorkbench.test.tsx src/features/settings/sources/browser-import/BrowserImportSection.test.tsx src/features/settings/sources/health/ScraperHealthDashboard.sourceChecks.test.tsx src/features/search-links/SearchLinksPage.test.tsx src/features/settings/config/SettingsConfig.test.ts src/features/settings/SettingsPage.sources.test.tsx src/features/settings/SettingsPage.load.test.tsx src/mocks/handlers/scraperInterviewCommands.test.ts src/shared/jobSourceGuidance.test.ts` | Passed on refresh: 109 tests across 13 files covering prompts, safe recovery text, visible-card Browser Import script coverage, collision-safe local listener setup, local review before saving, and fallback paths | Users see acknowledgement prompts, visible-page capture copy, local review before saving, safe recovery text, and manual fallback paths | No |
