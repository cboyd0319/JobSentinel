...
| Scraper construction and pipeline | `cargo test --test scraper_integration_test`, `cargo test --test scraping_pipeline_integration`, `cargo test --test api_contract_test`, and `cargo test --test scheduler_integration_test` | Passed on refresh: 25 scraper integration tests, 12 pipeline integration tests, 33 API contract tests, and 18 scheduler integration tests with 1 existing ignored SQLite-specific test. Refresh fixed stale integration-test `Config` fixtures after the external-AI config field was added. | Empty or failing sources do not block the local pipeline | No |
| Restricted-source UI and import contracts | `npm run test:run -- src/shared/jobSourceDiscoveryTaxonomy.test.ts src/shared/restrictedSourceTaxonomy.test.ts src/features/linkedin-workbench/linkedinWorkbenchPolicy.test.ts src/features/settings/sources/browser-import/BrowserImportButtonScript.test.ts src/features/linkedin-workbench/LinkedInWorkbench.test.tsx src/features/settings/sources/browser-import/BrowserImportSection.test.tsx src/features/settings/sources/health/ScraperHealthDashboard.sourceChecks.test.tsx src/features/search-links/SearchLinksPage.test.tsx src/features/settings/config/SettingsConfig.test.ts src/features/settings/SettingsPage.sources.test.tsx src/features/settings/SettingsPage.load.test.tsx src/dev-runtime/features/settings/sources/scraperHealth.test.ts src/dev-runtime/features/applications/interviewProgress.test.ts src/shared/jobSourceGuidance.test.ts` | Passed on refresh: 109 tests across 14 files covering prompts, safe recovery text, visible-page capture copy, collision-safe local listener setup, local review before saving, and fallback paths | Users see acknowledgement prompts, visible-page capture copy, local review before saving, safe recovery text, and manual fallback paths | No |
| LinkedIn-compatible restricted workflow | `cargo test --lib linkedin` and prior focused Chromium E2E for LinkedIn Workbench | Passed on refresh: 35 LinkedIn boundary tests covering local-only event ledger, disabled credential storage before keyring, fail-closed auth persistence, settings entry point, dashboard entry point, advisory-only config, pasted-text prefill, privacy reminder, no hidden monitoring, and no forced manual-session close | Add or update local records manually; use pasted link or manual entry if the browser path is unavailable | No |
| Browser Import local helper | `cargo test --lib bookmarklet` | Passed on refresh: 47 local helper tests covering loopback binding, origin checks, one-use tokens, URL safety, defensive headers, safe copy errors, import minimization, capped visible LinkedIn job-card batch queueing, review-before-save confirmation, invalid-batch no-partial-insert behavior, mixed-origin rejection, and no stored LinkedIn query context | Use pasted job links or manual entry if Browser Import cannot connect | No |
...
| Scrapling and Stygian Rust comparison | `cargo search scrapling --limit 10`, `cargo info scrapling`, `cargo info scrapling-fetch`, `cargo info scrapling-browser`, `cargo info scrapling-spider`, `cargo search stygian --limit 5`, and local checkout scans at Scrapling Rust `0d61c3e` and Python Scrapling `c0f012d` | Rechecked: Scrapling crates remain 0.2.0; Stygian crates are anti-detection/proxy/browser oriented; no v2.9.0 adoption because current adapters already use shared safe fetch and structured parsing, while these fetch/browser/spider paths add stealth, cookie/session, proxy, browser, crawler, redirect, challenge-solving, or storage behavior outside the release boundary | Keep existing adapters; reconsider parser-only Scrapling after release only with default features off and a failing fixture proving current parsing is insufficient; prefer clear source-changed fallbacks over heuristic field guesses | No |
...
Refresh drift found and corrected: source integration-test fixtures lagged the
current `Config` shape after the optional external-AI settings were added. The
