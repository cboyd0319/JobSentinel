...
| Dependency manifests, lockfiles, package-manager pins, or workflow actions | `npm run lint:deps`; for workflow action changes also run `npm run lint:actions`; before release run `npm run release:check-deps`. The npm package manager, all repo-declared direct npm packages, Cargo crates, and GitHub Actions must be exact-pinned to latest stable; resolved transitives must stay exact lockfile-pinned and latest-compatible. Do not force transitive updates outside upstream-supported ranges with overrides or patches. |
