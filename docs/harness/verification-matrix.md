...
| Authenticated activity ledger | User action required for every logged event, local-only storage, no DOM/network/storage inspection, no hidden browser-state inference, no silent restricted-site refresh, and ghost-job analysis limited to local user-confirmed records, pasted details, prior user snapshots, or public employer/ATS follow-through |
