...
- Keep deterministic checks as the real harness score:
  `npm run harness:score`, `npm run harness:benchmark`,
  `npm run harness:check`, `npm run doctor`, lint, tests, Rust checks, and
  E2E checks by risk.
...
For before/after harness tuning evidence, use:

```bash
npm run harness:benchmark
```

The benchmark combines score, restart metrics, next-best work, and a
recommendation without writing generated files unless `--output` is provided.
