...
| System `harness-engineering` skill | 2026-07-14 | Defines the authoritative five-subsystem standard contract, startup pack, state semantics, feedback loops, portability, architecture enforcement, and migration rules. | Adopted as the repository harness authority: concise `AGENTS.md`, native init launchers, root state, semantic checks, fail-closed routing, exhaustive file budgets, executable topology, revision-bound evidence, and disabled hosted projections. |
