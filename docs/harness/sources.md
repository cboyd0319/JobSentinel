...
| Walking Labs, `harness-creator` skill and scripts | 2026-05-31 | Packages a generic harness creator and structural validator around instructions, state, verification, scope, and lifecycle, using root compatibility files such as `feature_list.json`, `progress.md`, `session-handoff.md`, and `init.sh`. | Added `docs/harness/walkinglabs-harness-creator-skill-evaluation.md`; kept JobSentinel's richer `docs/plans/` state as source of truth, added `docs/plans/index.json`, and scored the repo-native tuple through `npm run harness:score`. |
