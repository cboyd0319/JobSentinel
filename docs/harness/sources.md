...
| Walking Labs, "Lecture 02. What a Harness Actually Is" | 2026-05-31 | Defines harness as instructions, tools, environment, state, and feedback; recommends scoring each subsystem, controlled variable exclusion tests, and analyzing Gulfs of Execution and Evaluation. | Historical evaluation remains archived. Structural scoring was retired because it did not prove executable behavior. |
| Walking Labs, `harness-creator` skill and scripts | 2026-05-31 | Packages a generic harness creator, validator, benchmark runner, and report renderer around instructions, state, verification, scope, and lifecycle. | Historical evaluation remains archived. Root standard-pack state was retained; duplicate score, benchmark, and scheduler ownership was deleted. |
...
| System `harness-engineering` skill | 2026-07-14 | Defines the authoritative five-subsystem standard contract, startup pack, state semantics, feedback loops, portability, architecture enforcement, and migration rules. | Adopted as the repository harness authority: concise `AGENTS.md`, native init launchers, canonical state, semantic checks, fail-closed routing, exhaustive file budgets, executable topology, revision-bound evidence, and disabled hosted projections. |
...
- Keep the root README reference anchor and `docs/references.md` source index
  complete. This source registry owns the curated research-packet URL
...
1. Re-check OpenAI, Anthropic, Martin Fowler, Thoughtworks, Red Hat, LangChain,
   Walking Labs, AgentPatterns.ai, Epsilla, Charles Anim, and AGENTS.md
   sources.
