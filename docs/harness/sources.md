...
| Walking Labs, "Lecture 02. What a Harness Actually Is" | 2026-05-31 | Defines harness as instructions, tools, environment, state, and feedback; recommends scoring each subsystem, improving the weakest one, running controlled variable exclusion tests, and analyzing Gulfs of Execution and Evaluation. | Added `docs/harness/archive/walkinglabs-lecture-02-evaluation.md`, `npm run doctor`, runtime pin files, and `npm run harness:score` for the lecture tuple. |
| Walking Labs, `harness-creator` skill and scripts | 2026-05-31 | Packages a generic harness creator, structural validator, benchmark runner, and report renderer around instructions, state, verification, scope, and lifecycle, using root compatibility files such as `feature_list.json`, `progress.md`, `session-handoff.md`, and `init.sh`. | Added `docs/harness/archive/walkinglabs-harness-creator-skill-evaluation.md`; kept JobSentinel's richer `docs/plans/` state as source of truth, added `docs/plans/index.json`, scored the repo-native tuple through `npm run harness:score`, and added `npm run harness:benchmark` for portable before/after benchmark output. |
