...
| Walking Labs, "Lecture 02. What a Harness Actually Is" | 2026-05-31 | Defines harness as instructions, tools, environment, state, and feedback; recommends scoring each subsystem, improving the weakest one, running controlled variable exclusion tests, and analyzing Gulfs of Execution and Evaluation. | Added `docs/harness/walkinglabs-lecture-02-evaluation.md` and `npm run doctor` for the weakest subsystem: environment readiness. |
| Walking Labs, `harness-creator` skill | 2026-05-31 | Packages a generic harness creator and structural validator around instructions, state, verification, scope, and lifecycle, using root compatibility files such as `feature_list.json`, `progress.md`, `session-handoff.md`, and `init.sh`. | Added `docs/harness/walkinglabs-harness-creator-skill-evaluation.md`; kept JobSentinel's richer `docs/plans/` state as source of truth instead of adding duplicate root files. |
