...
| P1 | Split `check-repo-bloat.mjs` into named sensors | Filesystem, tracked-artifact, dependency-ownership, and source-structure checks now live under `scripts/harness/checks/`; `scripts/check-repo-bloat.mjs` still exceeds 4,900 lines and covers stale docs, privacy logging, product phrasing, E2E-helper ownership, fixtures, and source security patterns. | Large mixed-purpose sensors are hard to review, hard to extend safely, and invite accidental broad edits. | Continue the registry split under `scripts/harness/checks/` with separate modules for docs drift, privacy logging, product copy, E2E-helper ownership, fixture quality, and release metadata. |
