...
| P1 | Split `check-repo-bloat.mjs` into named sensors | Filesystem, tracked-artifact, dependency-ownership, source-structure, E2E-helper, and product-framing checks now live under `scripts/harness/checks/`; `scripts/check-repo-bloat.mjs` still exceeds 4,800 lines and covers stale docs, privacy logging, fixtures, remaining product copy, and source security patterns. | Large mixed-purpose sensors are hard to review, hard to extend safely, and invite accidental broad edits. | Continue the registry split under `scripts/harness/checks/` with separate modules for docs drift, privacy logging, fixture quality, remaining product copy, and release metadata. |
