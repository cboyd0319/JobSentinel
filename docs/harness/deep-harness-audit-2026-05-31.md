...
| Priority | Improvement | Evidence | Risk | Recommended fix |
| -------- | ----------- | -------- | ---- | --------------- |
| P1 | Scan path aliases in frontend boundary checks | `tsconfig.json` defines `@/*`, while `check-frontend-boundaries.mjs` currently resolves relative imports only. | A future alias import can skip layer-boundary checks. | Teach the boundary checker to resolve `@/` paths from `tsconfig.json`. |
