...
| P1 | Split `check-repo-bloat.mjs` into named sensors | Filesystem, tracked-artifact, dependency-ownership, source-structure, E2E-helper, product-framing, and product-copy checks now live under `scripts/harness/checks/`; `scripts/check-repo-bloat.mjs` still exceeds 4,500 lines and covers stale docs, privacy logging, fixtures, broad-audience checks, technical-first copy, and source security patterns. | Large mixed-purpose sensors are hard to review, hard to extend safely, and invite accidental broad edits. | Continue the registry split under `scripts/harness/checks/` with separate modules for docs drift, privacy logging, fixture quality, broad-audience checks, technical-first copy, and release metadata. |
