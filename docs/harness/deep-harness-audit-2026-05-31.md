...
| P1 | Split `check-repo-bloat.mjs` into named sensors | Filesystem and tracked-artifact checks now live in `scripts/harness/checks/repo-artifacts.mjs`; `scripts/check-repo-bloat.mjs` still exceeds 5,100 lines and covers stale docs, privacy logging, product phrasing, dependency ownership, fixtures, and source security patterns. | Large mixed-purpose sensors are hard to review, hard to extend safely, and invite accidental broad edits. | Continue the registry split under `scripts/harness/checks/` with separate modules for docs drift, privacy logging, product copy, dependency ownership, fixture quality, and release metadata. |
