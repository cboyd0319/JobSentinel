...
`src-tauri/target/universal-apple-darwin/release/bundle/dmg/JobSentinel_2.7.5_no-account_universal.dmg`,
...
local-data permissions for the whole isolated app data tree. The `v2.7.5` Mac
...
2. **No-account public package path available** - Public macOS releases can use
   ...
   passing public verifier. The local `v2.7.5` no-account artifact must pass
