...
| Current wired runtime | Legacy `sentence-transformers/all-MiniLM-L6-v2` baseline while the Qwen3 backend is wired and validated |
...
FastEmbed may be used as an embedding backend implementation, but JobSentinel
owns model identity, revisions, cache integrity, runtime compatibility, vector
provenance, thresholds, and scoring semantics. Do not let a convenience
downloader decide production model bytes.
