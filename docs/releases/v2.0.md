...
2. **Resume Builder & Resume Match** for creating professional, readable resumes
...
Create professional, readable resumes directly in JobSentinel:

**Resume Builder:**

- 7-step wizard (contact, summary, experience, education, skills, preview, export)
- 5 readable templates: Classic, Modern, Technical, Executive, Military
...
- Job-post evidence review
- Resume-to-job alignment scoring
- Readability checking
- Completeness scoring
- 45+ action verbs for clearer bullet points
...
| Application readability | `analyze_resume_for_job`, `analyze_resume_format`, `extract_job_keywords`, `get_ats_power_words`, `improve_bullet_point` |
...
| `usajobs_api_key` | USAJobs source access |

### New Tauri Commands

Historical note: this section records the v2.0 launch shape. Current keyring
status entries include `available` as well as `key` and `exists`; use
`docs/security/KEYRING.md` as the current API source.

Credential management commands:

```typescript
// Store a credential
await invoke("store_credential", { key: "slack_webhook", value: "https://..." });

// Delete a credential
await invoke("delete_credential", { key: "smtp_password" });

// Check if credential exists
const hasWebhook = await invoke<boolean>("has_credential", { key: "slack_webhook" });

// Get all credential statuses
const status = await invoke<Array<{ key: string; exists: boolean }>>("get_credential_status");
```

Stored credential values are not returned to the renderer.
...
`src-tauri/Cargo.toml` owns current versions; this release note keeps the
credential dependency surface visible without pinning old backend feature flags.
...
- Legacy LinkedIn source tests expect source-policy disabled behavior
