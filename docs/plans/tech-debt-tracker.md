...
| -- | ---- | -------- | ---- | --------- | ------ |
| BLOAT-001 | Removing bloat and junk | Root contains disposable ignored artifacts after local runs (`dist/`, `test-results/`) plus needed local caches (`node_modules/`, `src-tauri/target/`) and generated Husky hook shims (`.husky/_`). Earlier tracked-file audits also found stale generated output outside root. | Junk hides meaningful changes, slows searches and verification, and makes agents more likely to inspect or preserve files that should not shape product behavior. | Sweep root and nested repo paths for disposable artifacts, delete safe ignored output after build/test runs, keep `node_modules/` and `src-tauri/target/` only as local verification caches, keep `.husky/_` while `core.hooksPath` points there, and continue tracked-file audits for stale generated files. | Open |
