...
- Current local care-plan plural follow-up in `bb11073e` treats care plan and
  ...
  requirement rows.
- Current local care-plan hyphen follow-up in `eef99504` adds care-plan and
  care-plans to that same local evidence group without duplicate requirement
  rows.
...
- Current local loan-processing hyphen follow-up in `84c08274` treats loan
  ...
  requirement rows.
- Current local selected-resume file-size follow-up in `19efc7ba` rejects
...
- Current local Application Assist certification quick-add follow-up in
  ...
  questions appear.
- Current local Application Assist travel quick-add follow-up in `cb2886df`
...
- Current local Application Assist saved-education review follow-up in
...
- Current local Application Assist background-screening quick-add follow-up in
  `50c8abfa` adds background check and drug screen to common saved-answer
  patterns, so users can prepare reusable answers before a hard screening
...
- Current local Application Assist language-screening review follow-up in
  `8657e403` flags bilingual, multilingual, and language-fluency requirements
  from saved job details and shows matching saved language answers in the hard
  question review when users have saved one. Verification passed: `npx vitest
  run src/components/automation/ApplicationPreview.test.tsx -t "language"`,
  `npx vitest run src/components/automation/ApplicationPreview.test.tsx`, `npx
  tsc --noEmit`, `npm run lint -- --quiet`, and `git diff --check`.
- Current local Application Assist language-screening quick-add follow-up in
  `a05a9ccd` adds bilingual language fluency to common saved-answer patterns,
  so users can prepare a reusable language answer before a hard language
  question appears. Verification passed: `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx -t "shows all common
  pattern buttons"`, `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`.
- Current local Application Assist physical-screening review follow-up in
  `2f22a8ac` flags lifting, standing, and physical-demand requirements from
  saved job details and shows matching saved physical-requirement answers in
...
- Current local Application Assist minimum-age review follow-up in `624f1c64`
  ...
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`.
- Current local Application Assist minimum-age quick-add follow-up in
  `94a26259` adds 18-years-of-age setup to common saved-answer patterns, so
  users can prepare a reusable age answer before a hard legal work-age question
...
- Current local Application Assist citizenship-screening quick-add follow-up in
  ...
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`.
- Current local Application Assist transportation-screening review follow-up
  in `79967c81` flags reliable-transportation and vehicle requirements from
  saved job details and shows matching saved transportation answers in the hard
  question review when users have saved one. Verification passed: `npx vitest
  run src/components/automation/ApplicationPreview.test.tsx -t
  "transportation"`, `npx vitest run
  src/components/automation/ApplicationPreview.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`.
- Current local Application Assist transportation-screening quick-add
  follow-up in `254982eb` adds reliable-transportation setup to common
  saved-answer patterns, so users can prepare a reusable transportation answer
  before a hard transportation or vehicle question appears. Verification
  passed: `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx -t "shows all common
  pattern buttons"`, `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`.
- Current local Application Assist driver's-license quick-add follow-up in
  `1b8b92dc` adds driver's-license setup to common saved-answer patterns, so
  users can prepare a reusable license answer before a hard driver's-license
  question appears. Verification passed: `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx -t "shows all common
  pattern buttons"`, `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`.
- Current local Application Assist overtime-screening review follow-up in
  `41e125bd` flags overtime requirements from saved job details and shows
...
- Current local Application Assist overtime-screening quick-add follow-up in
  ...
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`.
- Current local Application Assist holiday-screening quick-add follow-up in
  `209a94e2` adds holiday availability to common saved-answer patterns, so
  users can prepare a reusable holiday answer before a hard holiday question
  appears. Verification passed: `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx -t "shows all common
  pattern buttons"`, `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`.
- Current local Application Assist management-screening review follow-up in
  `695a2b12` flags management, supervision, and lead-experience requirements
  from saved job details and shows matching saved management answers in the
  ...
  "management"`, `npx vitest run
  src/components/automation/ApplicationPreview.test.tsx -t "managed"`, `npx
  vitest run src/components/automation/ApplicationPreview.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`.
- Current local Application Assist management-screening quick-add follow-up in
  `77a2ce6c` adds management experience to common saved-answer patterns, so
  users can prepare a reusable management answer before a hard management
  question appears. Verification passed: `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx -t "shows all common
  pattern buttons"`, `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
...
- Current local Resume Match full-time schedule constraint follow-up in
  ...
  src-tauri/Cargo.toml`, `cargo fmt --all --manifest-path
  src-tauri/Cargo.toml -- --check`, and `git diff --check`.
- Current local Resume Match overtime schedule constraint follow-up in
  `4369b5cf` recognizes required overtime availability as a local schedule hard
  constraint, caps missing required overtime evidence, and accepts overtime
  evidence without broad fuzzy matching. Verification passed: `cargo test --lib
  overtime --manifest-path src-tauri/Cargo.toml`, `cargo test --lib
...
- Current local relocation-term follow-up in `d4945db5` treats relocation,
  relocate, and willing to relocate as equivalent local hard-constraint
  evidence, so exact location wording differences do not create a false
  missing risk.
...
- Current local standing-period follow-up in `ce37de4c` treats stand for long
...
- Current local duty-backed evidence-strength follow-up in `fb7d484b` treats
  work or project evidence with clear task verbs tied to concrete requests,
...
- Current local CISSP follow-up in `cb7e461d` treats CISSP and Certified
  Information Systems Security Professional as equivalent local credential
  hard-constraint evidence.
...
- Current local business-administration bachelor follow-up in `2a3fd362`
...
- Current local engineering bachelor follow-up in `d355fd26` treats Bachelor
  ...
  requirement.
- Current local education bachelor follow-up in `fe6242f9` treats Bachelor of
...
- Current local fine-arts bachelor follow-up in `e019570c` treats Bachelor of
  ...
  requirement.
- Current local social-work bachelor follow-up in `293827cb` treats Bachelor
  of Social Work as local evidence for generic bachelor's degree requirements
...
- Current local master-degree title follow-up in `f032ae03` treats Master of
  ...
  requirements in browser/dev mocks.
- Current local business-administration master follow-up in `ff1c617e` treats
...
- Current local engineering master follow-up in `786a5974` treats Master of
  ...
  requirement.
- Current local education master follow-up in `22f1cd76` treats Master of
  Education as local evidence for generic master's degree requirements without
  treating education-specific job wording as a generic master's requirement.
- Current local fine-arts master follow-up in `90272a64` treats Master of Fine
  Arts as local evidence for generic master's degree requirements without
  treating fine-arts-specific job wording as a generic master's requirement.
- Current local social-work master follow-up in `2fb12f5e` treats Master of
  Social Work as local evidence for generic master's degree requirements
  without treating social-work-specific job wording as a generic master's
  requirement.
...
- Current state: plan updated with six source documents plus first-class
  ...
  Vital-sign hyphen work is committed in `c3a2375c`.
  ...
  Duty-backed evidence-strength work is committed in `fb7d484b`.
  ...
  Social-work bachelor work is committed in `293827cb`.
  ...
  Education master work is committed in `22f1cd76`.
  Fine-arts master work is committed in `90272a64`.
