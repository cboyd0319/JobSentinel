...
| Date | Status | Notes |
| ---- | ------ | ----- |
| 2026-07-13 | Milestone 2 in progress | Assigned cross-feature date, currency, contact-field, error-support, Tauri-call, job-fit, location, source-recommendation, and source-guidance behavior to named `src/shared/` boundaries. Dashboard owns job display formatting; Application Assist and Settings own private form validation. The latest source-guidance move made normalization private and passed 133 focused tests, three copy-policy tests, 2,953 frontend tests across 189 files, 766 script tests, the 809-module build, and 44 Dashboard and Applications E2E flows. No label, review guidance, user data, privacy, credential, consent, or external-side-effect boundary changed. |
...
- Shared date, US-dollar, and job-source guidance now have named owners under
  `src/shared/`; Dashboard owns job salary and description formatting. Three
  test-only generic formatters were deleted with the mixed root utility.
...
- Current state: repo-wide structure audited; target ownership and migration
  ...
  currency contracts; feature-private form and job display formatting; and
  mixed root utility cleanup all have passing focused, full frontend, build,
  repository, and E2E evidence.
