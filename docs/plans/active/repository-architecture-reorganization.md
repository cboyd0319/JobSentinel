...
| 2026-07-13 | Milestone 2 in progress | Finished feedback ownership: sanitized support-report generation is shared by app, Dashboard, and Settings error recovery; feedback commands and readable previews are private to Settings. Removed the unused Google Drive command, reducing the registered backend surface to 207, and renamed the non-persistent company summary fields with blocked/preferred terminology across TypeScript, mocks, and Rust. All 157 focused frontend tests, 40 policy tests, 54 Rust feedback tests, 2,933 frontend tests across 194 files, 766 script tests, the production build, repository gates, and 31 app-shell and Settings E2E flows pass. |
...
- Current state: repo-wide structure audited; target ownership and migration
  ...
  Error Reporting and support-report ownership, root contexts and hooks
  deletion, shared Tauri, location, source guidance, contact-field, date, and
...
- Next step: finish company research, score display, and external-AI ownership.
  Reduce `shared` ownership only from current consumer evidence.
