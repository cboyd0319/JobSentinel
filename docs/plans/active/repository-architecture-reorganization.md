...
| Date | Status | Notes |
| ---- | ------ | ----- |
| 2026-07-13 | Milestone 2 in progress | Assigned the cross-feature error-support contract to `src/shared/errorReporting/`, the product-neutral Tauri renderer client to `src/shared/tauri/commandClient.ts`, and the three cross-feature job-fit thresholds to `src/shared/jobMatchScore.ts`. Deleted six unused command-client exports, nine self-only tests, nine unused constants, and the mixed root constants bucket. Company research cache lifetime, interview duration, notification salary conversion, and posting-risk review thresholds now stay with their owners. The latest constants slice passed 231 focused tests, 3,032 frontend tests across 184 files, 766 script tests, the 804-module build, repository gates, and 60 job-filtering, Application Tracking, and Settings E2E flows. No behavior, user-facing copy, privacy boundary, or public wiki page changed. |
...
- Current state: repo-wide structure audited; target ownership and migration
  ...
  renderer client, and mixed root constants cleanup all have passing focused,
