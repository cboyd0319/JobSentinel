...
- Current local 2026 resume-guidance follow-up locks
  ...
  `38871425 Show saved education screening review` shows matching saved
  education, degree, diploma, bachelor's, master's, or high-school screening
  ...
  `50c8abfa Add screening background quick add` adds background check and drug
  screen to Application Assist's common saved-answer patterns, so users can
  prepare reusable answers before a hard screening question appears.
  ...
  `8657e403 Review language screening answers` flags bilingual, multilingual,
  and language-fluency requirements from saved job details and shows matching
  saved language answers in the hard question review when users have saved one.
  Verification passed: `npx vitest run
  src/components/automation/ApplicationPreview.test.tsx -t "language"`, `npx
  vitest run src/components/automation/ApplicationPreview.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`. The follow-up
  `a05a9ccd Add language screening quick add` adds bilingual language fluency
  to Application Assist's common saved-answer patterns, so users can prepare a
  reusable language answer before a hard language question appears.
  Verification passed: `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx -t "shows all common
  pattern buttons"`, `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  ...
  vitest run src/components/automation/ApplicationPreview.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`. The follow-up
  `40435528 Add physical screening quick add` adds physical requirements to
  Application Assist's common saved-answer patterns, so users can prepare a
  reusable physical-demand answer before a hard physical question appears.
  Verification passed: `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx -t "shows all common
  pattern buttons"`, `npx vitest run
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  ...
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`. The follow-up
  `79967c81 Review transportation screening answers` flags
  reliable-transportation and vehicle requirements from saved job details and
  shows matching saved transportation answers in the hard question review when
  users have saved one. Verification passed: `npx vitest run
  src/components/automation/ApplicationPreview.test.tsx -t "transportation"`,
  `npx vitest run src/components/automation/ApplicationPreview.test.tsx`, `npx
  tsc --noEmit`, `npm run lint -- --quiet`, and `git diff --check`. The
  follow-up
  `254982eb Add transportation screening quick add` adds
  reliable-transportation setup to Application Assist's common saved-answer
  patterns, so users can prepare a reusable transportation answer before a hard
  transportation or vehicle question appears. Verification passed: `npx vitest
  run src/components/automation/ScreeningAnswersForm.test.tsx -t "shows all
  ...
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`. The follow-up
  `41e125bd Review overtime screening answers` flags overtime requirements
  from saved job details and shows matching saved availability answers in the
  hard question review when users have saved one. Verification passed: `npx
  vitest run src/components/automation/ApplicationPreview.test.tsx -t
  ...
  src/components/automation/ScreeningAnswersForm.test.tsx`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, and `git diff --check`. The follow-up
  `695a2b12 Review management screening answers` flags management,
  supervision, and lead-experience requirements from saved job details and
  shows matching saved management answers in the hard question review when
  users have saved one. Verification passed: `npx vitest run
  src/components/automation/ApplicationPreview.test.tsx -t "management"`,
  `npx vitest run src/components/automation/ApplicationPreview.test.tsx -t
  ...
  --check`. The follow-up `187229e8 Add career resume evidence prompts` adds
  backend and dev/mock reminders for career-change and early-career job
  details, asking users to check transferable work, training, adjacent
  experience, projects or volunteer work, supervised responsibilities, and
  truthful transitions before using drafted alternative bullets. Verification
  passed: `cargo test --lib "career_" --manifest-path src-tauri/Cargo.toml`,
  ...
  diff --check`. Real resume/profile reference material remains private and
...
- Current local resume-alignment scoring follow-up is committed in
  ...
  same local evidence. The follow-up `94a51b03 Match guest service wording`
  extends that conservative evidence group to guest service and guest services,
  with backend and browser/dev mock parity. Verification passed: `cargo test
  --lib customer_support_service_equivalence --manifest-path
  ...
  clippy --manifest-path src-tauri/Cargo.toml -- -D warnings`, and `git diff
  --check`. The follow-up
  `c813eb12 Match onboarding orientation wording` treats onboarding, new hire
  orientation, and employee orientation as equivalent local service, HR, or
  training evidence in backend and dev/browser mocks without broad fuzzy
  matching. Verification passed: `cargo test --lib onboarding_orientation
  ...
  clippy --manifest-path src-tauri/Cargo.toml -- -D warnings`, and `git diff
  --check`. The follow-up
  `e59f7e82 Match training trained wording` treats training, trained, staff
  training, employee training, and team training as equivalent local training
  evidence in backend and dev/browser mocks without broad fuzzy matching.
  Verification passed: `cargo test --lib training_trained --manifest-path
  src-tauri/Cargo.toml`, `npx vitest run src/mocks/handlers.test.ts -t
  "analyzes resumes"`, `cargo test --lib ats_analyzer --manifest-path
  src-tauri/Cargo.toml`, `npx vitest run src/mocks/handlers.test.ts`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, `cargo fmt --all --manifest-path
  ...
  `ccfa88e9 Match medical record hyphen terms` adds medical-record and
  medical-records to that same local evidence group without duplicate
  requirement rows. The follow-up
  ...
  that same local evidence group without duplicate requirement rows. The follow-up
  `748a120e Match medication administration hyphen terms` treats medication
  administration and medication-administration as the same local evidence without
  ...
  rows. The follow-up
  `b219e11d Match case files hyphen terms` treats case files and case-files as
  the same local evidence without duplicate requirement rows. The follow-up
  ...
  passed: `cargo test --lib accounts_payable_receivable --manifest-path
  src-tauri/Cargo.toml`, `npx vitest run src/mocks/handlers.test.ts -t "A/P
  and A/R"`, `cargo test --lib ats_analyzer --manifest-path
  src-tauri/Cargo.toml`, `npx vitest run src/mocks/handlers.test.ts`, `npx tsc
  --noEmit`, `npm run lint -- --quiet`, `cargo fmt --all --manifest-path
  src-tauri/Cargo.toml -- --check`, `cargo clippy --manifest-path
  src-tauri/Cargo.toml -- -D warnings`, and `git diff --check`. The follow-up
  `432b49f7 Match cashier and cash handling evidence`
  ...
  `1dbee2aa Match budgeting and budget tracking` treats budgeting and budget
  tracking as equivalent local finance and operations evidence in backend and
  dev/browser mocks without broad fuzzy matching. Verification passed: `cargo
  test --lib budgeting_budget --manifest-path src-tauri/Cargo.toml`, `npx
  vitest run src/mocks/handlers.test.ts -t "budgeting and budget tracking"`,
  `cargo test --lib ats_analyzer --manifest-path src-tauri/Cargo.toml`, `npx
  vitest run src/mocks/handlers.test.ts`, `npx tsc --noEmit`, `npm run lint --
  --quiet`, `cargo fmt --all --manifest-path src-tauri/Cargo.toml -- --check`,
  `cargo clippy --manifest-path src-tauri/Cargo.toml -- -D warnings`, and `git
  diff --check`. The follow-up
  `eaff0b32 Match inventory stockroom wording` treats inventory, inventory
  control, inventory management, stock control, stock management, and stockroom
  as equivalent local operations evidence in backend and dev/browser mocks
  without broad fuzzy matching. Verification passed: `cargo test --lib
  ...
  mocks without broad fuzzy matching. Verification passed: `cargo test --lib
  procurement_purchasing --manifest-path src-tauri/Cargo.toml`, `npx vitest run
  src/mocks/handlers.test.ts -t "procurement and purchasing"`, `cargo test
  --lib ats_analyzer --manifest-path src-tauri/Cargo.toml`, `npx vitest run
  src/mocks/handlers.test.ts`, `npx tsc --noEmit`, `npm run lint -- --quiet`,
  `cargo fmt --all --manifest-path src-tauri/Cargo.toml -- --check`, `cargo
  clippy --manifest-path src-tauri/Cargo.toml -- -D warnings`, and `git diff
  ...
  `3ba9f95c Review remote hybrid resume constraints` recognizes required
  remote-work and hybrid-work language as local location-style hard constraints,
  caps missing required hybrid evidence, and accepts related remote role
  ...
  src-tauri/Cargo.toml`, `cargo fmt --all --manifest-path
  src-tauri/Cargo.toml -- --check`, and `git diff --check`. The follow-up
  `4369b5cf Review overtime resume constraints` recognizes required overtime
  availability as a local schedule hard constraint, caps missing required
  overtime evidence, and accepts overtime evidence without broad fuzzy
  matching. Verification passed: `cargo test --lib overtime --manifest-path
  ...
  `d814a00a Review holiday resume constraints` recognizes required holiday
  availability as a local schedule hard constraint, caps missing required
  holiday evidence, and accepts holiday shift evidence without broad fuzzy
  matching. Verification passed: `cargo test --lib holiday --manifest-path
  src-tauri/Cargo.toml`, `cargo test --lib ats_analyzer --manifest-path
  src-tauri/Cargo.toml`, `cargo fmt --all --manifest-path src-tauri/Cargo.toml
  -- --check`, `cargo clippy --manifest-path src-tauri/Cargo.toml -- -D
  warnings`, and `git diff --check`. The follow-up
  ...
  requirement. The
  follow-up `e019570c Match fine arts bachelor term` treats Bachelor of Fine
  Arts as local evidence for generic bachelor's degree requirements without
  treating fine-arts-specific job wording as a generic bachelor's requirement.
  The follow-up `293827cb Match social work bachelor term` treats Bachelor of
  Social Work as local evidence for generic bachelor's degree requirements
  without treating social-work-specific job wording as a generic bachelor's
  requirement. The
  follow-up
  ...
  requirement. The
  follow-up `786a5974 Match engineering master term` treats Master of
...
- Current local qualified-experience heading follow-up in
  ...
  src-tauri/Cargo.toml -- --check`, `cargo clippy --manifest-path
  src-tauri/Cargo.toml -- -D warnings`, and `git diff --check`.
- Current local academic-heading follow-up in
  `efdfe888 Recognize academic resume headings` treats Academic Background,
  Academic History, and Education Background as standard readable-resume
  structure and education evidence instead of generic resume text.
  Verification passed: `cargo test --lib academic_headings --manifest-path
  src-tauri/Cargo.toml`, `cargo test --lib ats_analyzer --manifest-path
  src-tauri/Cargo.toml`, `cargo fmt --all --manifest-path src-tauri/Cargo.toml
  -- --check`, `cargo clippy --manifest-path src-tauri/Cargo.toml -- -D
  warnings`, and `git diff --check`.
